...
async function comprehensiveServerTest() {
  ...
  if (portInUse) {
    console.log('⚠️ Port 5000 is already in use. This suggests another application is running on that port.');
    console.log('Your server might be running, but let\'s perform more tests to be certain.\n');
  } else {
    console.log('✅ Port 5000 is available. If your server is supposed to be running, it might not be binding correctly.\n');
  }
  ...
  if (portInUse && !tcpWorks) {
    ...
    console.log('⚠️ PARTIAL ISSUE: Server is listening on port 5000 but HTTP requests are failing.');
