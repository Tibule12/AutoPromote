...
async function checkStatus() {
  ...
  scheduleSnap.forEach(doc => console.log('  -', doc.id, doc.data().platform, doc.data().startTime));

  // 3. Promotion tasks
  const taskSnap = await db.collection('promotion_tasks').where('uid', '==', uid).get();
  console.log(`Promotion tasks: ${taskSnap.size}`);
  taskSnap.forEach(doc => console.log('  -', doc.id, doc.data().type, doc.data().platform || 'youtube', doc.data().status));
