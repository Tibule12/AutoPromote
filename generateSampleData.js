...
async function generateUsers(count = 20) {
  ...
  const names = [
    'John Smith', 'Jane Doe', 'Michael Johnson', 'Emma Williams', 
    'Robert Brown', 'Olivia Jones', 'William Davis', 'Sophia Miller',
    'James Wilson', 'Isabella Moore', 'David Taylor', 'Mia Anderson',
    'Joseph Thomas', 'Charlotte Jackson', 'Charles White', 'Amelia Harris',
    'Daniel Martin', 'Harper Thompson', 'Matthew Garcia', 'Evelyn Martinez'
  ];
...
async function generateContent(count = 30) {
  ...
  const titles = [
    'Getting Started with AutoPromote', 'How to Maximize Your Reach',
    'Best Practices for Content Creation', 'Understanding Analytics',
    'Promotion Strategies That Work', 'Building Your Brand',
    'Engagement Tactics for Social Media', 'Creating Viral Content',
    'The Ultimate Guide to Digital Marketing', 'Content Scheduling Tips',
    'Measuring ROI of Your Promotions', 'Audience Targeting Guide',
    'Cross-Platform Promotion', 'Visual Content Strategies',
    'Using Hashtags Effectively', 'SEO Tips for Content'
  ];
  ...
  for (let i = 0; i < count; i++) {
    ...
    const contentData = {
      ...
      tags: Array.from({ length: randomInt(1, 5) }, () => 
        randomChoice(['marketing', 'social media', 'promotion', 'strategy', 'content', 'digital', 'brand', 'seo', 'analytics'])
      )
    };
...
async function generatePromotions(count = 25) {
  ...
  for (let i = 0; i < count; i++) {
    const createdAt = randomDate(new Date(2022, 0, 1), new Date());
    const startDate = randomDate(createdAt, new Date(createdAt.getTime() + 1000 * 60 * 60 * 24 * 30));
    ...
    const promotionData = {
      ...
      targeting: {
        ...
        locations: Array.from({ length: randomInt(1, 3) }, () => 
          randomChoice(['United States', 'Canada', 'United Kingdom', 'Australia', 'Germany', 'France', 'Japan'])
        ),
        interests: Array.from({ length: randomInt(1, 4) }, () => 
          randomChoice(['technology', 'sports', 'fashion', 'travel', 'food', 'music', 'movies', 'business'])
        )
      }
    };
...
async function generateActivities(count = 50) {
  ...
  const actionTypes = [
    'user_registered', 'user_login', 'content_created', 'content_updated',
    'promotion_created', 'promotion_started', 'promotion_ended', 'content_viewed',
    'settings_updated', 'profile_updated', 'payment_processed', 'subscription_changed'
  ];
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (actionType.includes('user')) {
      ...
      details = {
        userId: `user_${randomInt(1, 20)}`,
        status: 'success',
        change: randomChoice(['plan upgrade', 'plan downgrade', 'payment method', 'account settings'])
      };
    }
