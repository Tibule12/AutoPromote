...
const TestConsole = () => {
  ...
  const testAdminQueries = async () => {
    ...
    const queries = [
      {
        name: "Recent users",
        execute: () => firestore.collection("users").orderBy("createdAt", "desc").limit(10).get(),
      },
      {
        name: "Content metrics",
        execute: () => firestore.collection("content").orderBy("views", "desc").limit(5).get(),
      },
      {
        name: "Recent activities",
        execute: () =>
          firestore.collection("activities").orderBy("timestamp", "desc").limit(20).get(),
      },
      {
        name: "Active promotions",
        execute: () => firestore.collection("promotions").where("status", "==", "active").get(),
      },
      {
        name: "Analytics summary",
        execute: () => firestore.collection("analytics").doc("summary").get(),
      },
    ];
  ...
  return (
    <div className="test-console">
      ...
      <div className="test-controls">
        <button onClick={runAllTests} disabled={loading} className="test-button run-all">
      ...
      <div className="test-results">
        ...
        <div className={`result-section ${testStatus.connection}`}>
          ...
          {expandedSections.connection && (
            <div className="section-content">
              {testStatus.connection === "passed" && <p>Successfully connected to Firestore.</p>}
        ...
        <div className={`result-section ${testStatus.collections}`}>
          <div className="section-header" onClick={() => toggleSection("collections")}>
        ...
        <div className={`result-section ${testStatus.queries}`}>
          <div className="section-header" onClick={() => toggleSection("queries")}>
            <h2>Queries Test</h2>
        ...
        <p>
          ...
          testStatus.queries === "passed" ? (
            <span className="summary-success">
              ✅ All tests passed! The database and admin dashboard are fully integrated.
            </span>
