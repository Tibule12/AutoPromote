...
async function testAdminQueries() {
  ...
  const queries = [
    {
      name: "Recent users",
      execute: () => db.collection("users").orderBy("createdAt", "desc").limit(10).get(),
    },
    {
      name: "Content metrics",
      execute: () => db.collection("content").orderBy("views", "desc").limit(5).get(),
    },
    {
      name: "Recent activities",
      execute: () => db.collection("activities").orderBy("timestamp", "desc").limit(20).get(),
    },
    {
      name: "Active promotions",
      execute: () => db.collection("promotions").where("status", "==", "active").get(),
    },
    {
      name: "Analytics summary",
      execute: () => db.collection("analytics").doc("summary").get(),
    },
  ];
...
async function runTests() {
  ...
  if (results.collections) {
    console.log(
      `Collections: ${results.collections.missing.length === 0 ? "✅ PASSED" : "❌ FAILED"}`
    );
