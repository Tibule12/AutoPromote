...
if (process.env.FIREBASE_SERVICE_ACCOUNT) {
  ...
  try {
    ...
    if (missingFields.length > 0) {
      console.log(
        `${colors.red}✗${colors.reset} Service account JSON is missing required fields: ${missingFields.join(", ")}`
      );
    ...
    console.log(
      `${colors.red}✗${colors.reset} Failed to parse FIREBASE_SERVICE_ACCOUNT as JSON: ${error.message}`
    );
    useServiceAccountJson = false;
  }
} else {
  console.log(
    `${colors.yellow}⚠${colors.reset} FIREBASE_SERVICE_ACCOUNT is not set, checking for individual credentials`
  );
}
...
if (!useServiceAccountJson) {
  ...
  if (missingCredFields.length > 0) {
    console.log(
      `${colors.red}✗${colors.reset} Missing required Firebase credential fields: ${missingCredFields.join(", ")}`
    );
  } else {
  ...
  return new Promise(resolve => {
    ...
    https
      .get("https://www.google.com", res => {
        ...
        if (timeDiff > 150000) {
          // 150 seconds - Firebase typically allows 5 minute clock skew
          console.log(
            `${colors.red}✗${colors.reset} Server clock is significantly out of sync (${Math.round(timeDiff / 1000)} seconds difference)`
          );
          console.log(
            `${colors.yellow}⚠${colors.reset} This may cause JWT verification to fail. Please synchronize your server clock.`
          );
        } else if (timeDiff > 30000) {
          // 30 seconds
          console.log(
            `${colors.yellow}⚠${colors.reset} Server clock is slightly out of sync (${Math.round(timeDiff / 1000)} seconds difference)`
          );
          console.log(
            `${colors.blue}ℹ${colors.reset} This should be within Firebase's 5-minute tolerance for JWT verification, but synchronizing is recommended.`
          );
        } else {
      ...
      .on("error", err => {
        console.log(
          `${colors.yellow}⚠${colors.reset} Could not check time synchronization: ${err.message}`
        );
        resolve();
      });
  });
...
async function initializeFirebase() {
  ...
  if (admin.apps.length > 0) {
    console.log(
      `${colors.yellow}⚠${colors.reset} Firebase Admin SDK is already initialized. Deleting all apps...`
    );
  ...
  try {
    ...
    if (error.message.includes("invalid_grant")) {
      ...
      if (error.message.includes("Invalid JWT Signature")) {
        console.log(
          `${colors.yellow}⚠${colors.reset} Invalid JWT Signature error detected. This likely means:`
        );
        ...
        console.log(
          `${colors.yellow}⚠${colors.reset} Not a valid service account error. This likely means:`
        );
...
async function testFirebaseServices(app) {
  if (!app) {
    console.log(
      `${colors.red}✗${colors.reset} Skipping service tests due to initialization failure`
    );
    return;
  }
  ...
  try {
    ...
    console.log(
      `${colors.blue}ℹ${colors.reset} Found ${listUsersResult.users.length} user(s) in the project`
    );
  } catch (error) {
    console.log(
      `${colors.red}✗${colors.reset} Authentication service test failed: ${error.message}`
    );
  }
  ...
  try {
    ...
    if (
      ...
      console.log(
        `${colors.yellow}⚠${colors.reset} Firestore might not be enabled for this project.`
      );
  ...
  try {
    ...
    console.log(
      `${colors.blue}ℹ${colors.reset} Default bucket ${exists ? "exists" : "does not exist"}`
    );
    ...
    if (error.message.includes("PERMISSION_DENIED") || error.message.includes("does not exist")) {
      console.log(
        `${colors.yellow}⚠${colors.reset} Firebase Storage might not be enabled for this project.`
      );
...
function generateRecommendations() {
  ...
  console.log(
    '2. If you received "Invalid JWT Signature" errors, generate a new service account key'
  );
