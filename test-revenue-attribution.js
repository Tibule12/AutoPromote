...
async function testRevenueAttribution() {
  try {
    ...
    await db
      ...
      .doc(contentId)
      .update({
        revenue: admin.firestore.FieldValue.increment(2.5),
      });
      ...
      .set(
        {
          contentId,
          userId,
          totalRevenue: admin.firestore.FieldValue.increment(2.5),
          updatedAt: admin.firestore.FieldValue.serverTimestamp(),
        },
        { merge: true }
      );
    ...
    console.log(
      `✓ getRevenueSummary: user revenue = ${userTotal}, platform revenue = ${platformTotal}`
    );
