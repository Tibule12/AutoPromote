...
// Firebase client config - read from env vars to avoid hard-coded keys
...
async function createUserWithAdminSDK() {
  try {
    ...
    console.log(
      "❌ Error creating user with Admin SDK:",
      error.code || "",
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          console.log(
            `❌ Login still failed for ${user.email} after password reset:`,
            loginError.code
          );
        }
      } catch (userError) {
        if (userError.code === "auth/user-not-found") {
          ...
          try {
            ...
            try {
              ...
              console.log(
                `❌ Login still failed for newly created ${user.email}:`,
                finalLoginError.code
              );
            }
          } catch (createError) {
            console.log(
...
async function runAuthFix() {
  try {
    ...
    if (testUser.success) {
      console.log(
        "Email (masked):",
        testUser.email ? "***" + String(testUser.email).slice(-8) : "<none>"
      );
      console.log("Password: <REDACTED>");
    }
    console.log("\nNEW ADMIN USER:");
    if (adminUser.success) {
      console.log(
        "Email (masked):",
        adminUser.email ? "***" + String(adminUser.email).slice(-8) : "<none>"
      );
      console.log("Password: <REDACTED>");
    }
  } catch (error) {
