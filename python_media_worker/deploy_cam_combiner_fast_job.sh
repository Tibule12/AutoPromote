...
JOB_NAME="${MULTICAM_FAST_RENDER_JOB_NAME:-cam-combiner-render-fast-job}"
...
# Do not double-charge a failed 8-vCPU render. The durable job remains
# retryable by the user after the failure is visible and understood.
...
gcloud run jobs describe "$JOB_NAME" \
  --project "$PROJECT_ID" \
  --region "$REGION" \
  --format='table(metadata.name,spec.template.template.spec.containers[0].image,spec.template.template.spec.containers[0].resources.limits.cpu,spec.template.template.spec.containers[0].resources.limits.memory)'
