...
gcloud run deploy "$SERVICE_NAME" \
  ...
  --min-instances 0 \
  --max-instances 3 \
  --set-env-vars "NODE_ENV=production,FIREBASE_STORAGE_BUCKET=${BUCKET},MULTICAM_INGEST_BUCKET=${BUCKET},MULTICAM_UPLOAD_FIREBASE=true,MULTICAM_MASTER_RETENTION_DAYS=7,MULTICAM_BETA_MAX_SECONDS=10800,MULTICAM_BETA_MAX_SEGMENTS=450,MULTICAM_RENDER_CHECKPOINTS_ENABLED=true,MULTICAM_RENDER_CHECKPOINT_SECONDS=300,MULTICAM_CHECKPOINT_DURATION_TOLERANCE_SECONDS=0.10,MULTICAM_EXECUTION_MODE=cloud_run_jobs,ENABLE_LOCAL_MEDIA_OUTPUT_FALLBACK=false,MULTICAM_REUSE_VIDEO_ONLY_SYNC_AUDIT=1,WHISPER_ENGINE=faster,FASTER_WHISPER_DEVICE=cpu,FASTER_WHISPER_COMPUTE_TYPE=int8,ALLOW_RUNTIME_DEPENDENCY_INSTALL=false" \
