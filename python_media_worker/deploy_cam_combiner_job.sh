...
gcloud run jobs deploy "$JOB_NAME" \
  ...
  --max-retries 1 \
  --task-timeout 4h \
  --set-env-vars "NODE_ENV=production,FIREBASE_STORAGE_BUCKET=${BUCKET},MULTICAM_INGEST_BUCKET=${BUCKET},MULTICAM_UPLOAD_FIREBASE=true,MULTICAM_MASTER_RETENTION_DAYS=7,MULTICAM_BETA_MAX_SECONDS=10800,MULTICAM_BETA_MAX_SEGMENTS=450,MULTICAM_RENDER_CHECKPOINTS_ENABLED=true,MULTICAM_RENDER_CHECKPOINT_SECONDS=300,MULTICAM_CHECKPOINT_DURATION_TOLERANCE_SECONDS=0.10,ENABLE_LOCAL_MEDIA_OUTPUT_FALLBACK=false,MULTICAM_REUSE_VIDEO_ONLY_SYNC_AUDIT=1,MULTICAM_SOURCE_PREP_CONCURRENCY=2,MULTICAM_VISUAL_PROXY_CONCURRENCY=2,MULTICAM_JOB_MAX_RETRIES=1,WHISPER_ENGINE=faster,FASTER_WHISPER_DEVICE=cpu,FASTER_WHISPER_COMPUTE_TYPE=int8,ALLOW_RUNTIME_DEPENDENCY_INSTALL=false,MULTICAM_JOB_CALLBACK_URL=${CALLBACK_URL}" \
