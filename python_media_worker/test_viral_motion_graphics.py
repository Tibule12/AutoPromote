...
from viral_motion_graphics import (
    ...
    generate_transparent_motion_overlay,
)
...
class MotionContractTests(unittest.TestCase):
    def test_render_endpoint_rejects_unknown_designs_before_queueing(self):
        ...
        with self.assertRaises(HttpError) as error:
            asyncio.run(namespace["render_viral_clip"](request))
        self.assertEqual(error.exception.status_code, 422)
        implementation.assert_not_called()
        request.motionGraphics = {"version": 1, "scenes": []}
        asyncio.run(namespace["render_viral_clip"](request))
    ...
    def test_every_motion_preset_stays_inside_action_safe_edges(self):
        for preset in (value for value in PRESETS if value != "watermark"):
            scene = normalize_motion(dict(
                preset=preset, text="A STRONG IDEA", secondary="SAFE AT EVERY EDGE",
                duration=4, x=5, y=95, endX=95, endY=5,
                scale=1.4, endScale=1.4, rotation=-45, endRotation=45,
                canvasMode="overlay",
            ))
            for width, height in ((640, 360), (360, 640), (480, 480), (432, 540)):
                for time in (.1, 1, 2, 3, 3.9):
                    with self.subTest(preset=preset, width=width, height=height, time=time):
                        left, top, right, bottom = draw_motion_frame(
                            [scene], time, width, height
                        ).getbbox()
                        self.assertGreaterEqual(left, math.floor(width * .039))
                        self.assertGreaterEqual(top, math.floor(height * .039))
                        self.assertLessEqual(right, math.ceil(width * .961))
                        self.assertLessEqual(bottom, math.ceil(height * .961))

    def test_brand_reveal_is_a_standalone_frontend_composition_by_default(self):
        scene = normalize_motion(dict(preset="brand_reveal", startTime=.5, duration=2))
        self.assertEqual(scene["canvasMode"], "standalone")
        self.assertEqual(scene["canvasColor"], "#050713")
        frame = draw_motion_frame([scene], 1, 320, 180)
        self.assertEqual(frame.getpixel((0, 0)), (5, 7, 19, 255))
        self.assertEqual(frame.getbbox(), (0, 0, 320, 180))
        self.assertIsNone(draw_motion_frame([scene], 2.5, 320, 180).getbbox())
...
const scenes=["card","editorial"].flatMap(design=>MOTION_PRESETS.map(p=>normalizeMotion({{design,id:p.id,preset:p.id,text:"A STRONG IDEA",secondary:"THE OTHER IDEA",value:-125,prefix:"R",startTime:2,duration:4,x:30,endX:65,y:25,endY:70,rotation:-10,endRotation:20}})));
...
class MotionRenderTests(unittest.TestCase):
    ...
    def test_production_overlay_preserves_alpha_aspect_and_speed_clock(self):
        with tempfile.TemporaryDirectory() as temp:
            output = str(Path(temp)/"overlay.mov")
            generate_transparent_motion_overlay(
                {"version": 1, "scenes": [dict(id="title", preset="title", text="REAL EDIT", startTime=.4, duration=.8)]},
                1, output, 30, 320, 180,
                [{"start_time": 0, "end_time": 2, "rate": 2}],
            )
            info = probe(output)
            self.assertEqual((info["streams"][0]["width"], info["streams"][0]["height"]), (320, 180))
            self.assertAlmostEqual(float(info["format"]["duration"]), 1, delta=.04)
            def alpha(time):
                raw = run(["ffmpeg", "-v", "error", "-ss", str(time), "-i", output, "-frames:v", "1",
                           "-f", "rawvideo", "-pix_fmt", "rgba", "pipe:1"])
                return np.frombuffer(raw, dtype=np.uint8).reshape(180, 320, 4)[:, :, 3]
            self.assertEqual(alpha(.1).max(), 0)
            self.assertGreater(alpha(.35).max(), 100)
            self.assertEqual(alpha(.8).max(), 0)
