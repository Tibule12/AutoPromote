...
console.log(
  `Format: ${testToken.startsWith("eyJ") ? "✅ Begins with eyJ (likely JWT)" : "❌ Does not begin with eyJ (not a standard JWT)"}`
);
console.log(
  `Preview: ${testToken.substring(0, 10)}...${testToken.substring(testToken.length - 5)}`
);
console.log("---------------------------------------------------------");
...
auth
  ...
  .catch(error => {
    ...
    if (error.code === "auth/argument-error") {
      ...
      console.log(
        "   Custom tokens need to be exchanged for ID tokens before use with verifyIdToken()."
      );
