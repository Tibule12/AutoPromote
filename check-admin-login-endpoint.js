...
const req = http.request(options, (res) => {
  ...
  if (res.headers['access-control-allow-origin']) {
    console.log('✅ CORS is properly configured for preflight with Access-Control-Allow-Origin header');
  } else {
    console.log('❌ CORS header "Access-Control-Allow-Origin" is missing in preflight!');
  }
  
  if (res.headers['access-control-allow-methods']) {
    console.log('✅ CORS is properly configured for preflight with Access-Control-Allow-Methods header');
  ...
  if (res.headers['access-control-allow-headers']) {
    console.log('✅ CORS is properly configured for preflight with Access-Control-Allow-Headers header');
