...
function findRoute(router, method, path) {
  ...
  const lastIndex =
    layer.route.stack && layer.route.stack.length ? layer.route.stack.length - 1 : 0;
...
async function invoke(handler, { body = {}, userId = "test-user" } = {}) {
  return await new Promise(resolve => {
    ...
    try {
      ...
      resolve({
        status: 500,
        body: { error: e && e.message ? e.message : String(e), stack: e && e.stack },
      });
    }
  });
}
...
describe("payments routes smoke tests", () => {
  test("/status returns ok true", async () => {
    ...
    if (res1.status !== 200) {
      console.log("status error body:", res1.body);
    }
