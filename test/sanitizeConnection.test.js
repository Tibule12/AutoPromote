...
(async () => {
  try {
    const doc = { tokens: { access_token: 'secret', refresh_token: 'secret2' }, access_token: 'x', refresh_token: 'y', client_secret: 'z', meta: { tokens: { access_token: 'm' }, other: 'val' } };
    const sanitized = sanitizeConnectionForApi(doc);
    if (sanitized.tokens || sanitized.access_token || sanitized.refresh_token || sanitized.client_secret) {
