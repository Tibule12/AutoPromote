...
describe("Platform metadata simulate endpoints", () => {
  ...
  async function simAndGet(platform, meta) {
    ...
    try {
      const d = await db
        ...
        .doc(platform)
        .get();
      if (d.exists) console.log("Stored connection meta for platform", platform, d.data());
      else console.log("No stored connection doc for", platform);
    } catch (e) {
      console.log("Error reading stored connection doc for", platform, e && e.message);
    }
  ...
  it("returns Spotify metadata after simulate", async () => {
    const res = await simAndGet("spotify", {
      display_name: "SimUser",
  ...
  it("returns Discord metadata after simulate", async () => {
    const res = await simAndGet("discord", {
      display_name: "DUser",
      guilds: [{ id: "g1", name: "G1" }],
    });
    expect(res.meta).toBeDefined();
  ...
  it("returns LinkedIn metadata after simulate", async () => {
    const res = await simAndGet("linkedin", {
      profile: { localizedFirstName: "L" },
  ...
  it("returns Pinterest metadata after simulate", async () => {
    const res = await simAndGet("pinterest", {
      display_name: "PUser",
