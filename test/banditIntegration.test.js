...
(async function run(){
  ...
  try {
    const { recordSelectionOutcome, computeSuggestedWeights } = require('../src/services/banditTuningService');
    // Inject synthetic outcome samples
    for (let i=0;i<60;i++) {
      await recordSelectionOutcome({ contentId:'c1', platform:'test', variant:'v'+(i%3), rewardCtr: Math.random()*0.1 + (i%3===0?0.05:0), rewardQuality: 0.4 + Math.random()*0.2, rewardReach: 0.05 + Math.random()*0.05 });
    }
