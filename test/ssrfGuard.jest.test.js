...
describe("ssrfGuard", () => {
  ...
  test("safeFetch rejects redirect to private IP", async () => {
    ...
    const fetchFn = jest.fn().mockResolvedValue({
      status: 302,
      headers: {
        get: h => (h.toLowerCase() === "location" ? "https://192.168.1.5/" : undefined),
      },
    });
    await expect(safeFetch("https://example.com/", fetchFn)).rejects.toThrow(
      /ssrf_blocked:redirect_private_ip/
    );
  });
});
