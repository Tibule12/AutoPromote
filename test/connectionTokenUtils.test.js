...
(async () => {
  try {
    ...
    if (!t1 || t1.access_token !== 'a1' || t1.refresh_token !== 'r1') throw new Error('object tokens test failed');
    ...
    if (!t2 || t2.access_token !== 'a2' || t2.refresh_token !== 'r2') throw new Error('encrypted tokens string failed');

    // Case 3: encrypted_access_token field
    const accessEnc = encryptToken('a3');
    const refreshEnc = encryptToken('r3');
    const doc3 = { encrypted_access_token: accessEnc, encrypted_refresh_token: refreshEnc, expires_in: 3600 };
    const t3 = tokensFromDoc(doc3);
    if (!t3 || t3.access_token !== 'a3' || t3.refresh_token !== 'r3' || t3.expires_in !== 3600) throw new Error('encrypted fields test failed');
