...
describe('Diagnostics Remediation', () => {
  ...
  afterAll(async () => { try { await server.close(); } catch (e) {} });
  ...
  it('applies remediation for missing admin collection', async () => {
    ...
    const { result } = await agent.get('/api/diagnostics/scan?dashboard=admin')
    ...
    const store = await agent.get('/api/diagnostics/scan?dashboard=admin&store=1').set('Authorization','Bearer test-token-for-adminUser');
    ...
    const res = await agent.post(`/api/diagnostics/scans/${scanDoc.id}/remediate`).set('Authorization','Bearer test-token-for-adminUser').send({ checks: ['admin'] }).expect(200);
