...
ssrf.safeFetch = (url, fetchFn, opts) => {
  if (String(url).includes('/oauth/access_token')) {
    return Promise.resolve({ ok: true, json: async () => ({ access_token: 'FB_ACCESS', token_type: 'bearer', expires_in: 3600 }) });
  }
  if (String(url).includes('/me/accounts')) {
    return Promise.resolve({ ok: true, json: async () => ({ data: [{ id: 'page-1', name: 'MyPage', access_token: 'PAGE_ACCESS' }] }) });
  }
  if (String(url).includes('/' + 'page-1')) {
    return Promise.resolve({ ok: true, json: async () => ({ id: 'page-1', name: 'MyPage', instagram_business_account: { id: 'ig-123' } }) });
  }
