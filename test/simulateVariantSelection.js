...
function rotationSelect(stats, idx){ return idx % stats.length; }
...
function runSimulation({ variants=5, rounds=5000, trueRates, strategy='ucb1' }){
  ...
  for (let r=0;r<rounds;r++){
    let choice;
    if (strategy==='ucb1') choice = ucb1Select(stats); else choice = rotationSelect(stats, rotIdx++);
    // Bernoulli reward
    const reward = Math.random() < trueRates[choice] ? 1 : 0;
    stats[choice].trials++; stats[choice].reward += reward;
  }
...
function main(){
  ...
  function freq(stats){ return stats.map(s=> s.trials); }
