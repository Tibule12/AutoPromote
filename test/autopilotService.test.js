...
describe('autopilotService.decideAutoApply', () => {
  test('recommends a winner when above threshold', () => {
    const testData = {
      ...
      variants: [
        { id: 'A', metrics: { views: 1200, conversions: 50, engagement: 200 }, promotionSettings: { budget: 100 } },
        { id: 'B', metrics: { views: 300, conversions: 5, engagement: 100 }, promotionSettings: { budget: 100 } }
      ]
    };
  ...
  test('does not recommend when disabled or sample size too low', () => {
    const testData = {
      ...
      variants: [
        { id: 'A', metrics: { views: 10, conversions: 1, engagement: 2 }, promotionSettings: { budget: 10 } },
        { id: 'B', metrics: { views: 12, conversions: 1, engagement: 2 }, promotionSettings: { budget: 10 } }
      ]
    };
    const decision = autopilotService.decideAutoApply(testData);
...
describe('autopilotService.canApplyAuto', () => {
  ...
  test('returns true when requiresApproval is true and approvedBy exists', () => {
    const testData = { autopilot: { enabled: true, requiresApproval: true, approvedBy: 'admin123' } };
    const ok = autopilotService.canApplyAuto(testData);
    expect(ok).toBe(true);
  });
});
...
describe('autopilotService.buildCanaryScheduleData', () => {
  test('builds schedule from previous budget when present', () => {
    ...
    const schedule = service.buildCanaryScheduleData(fakeTestData, winningVariant, { canaryPct: 10, rampHours: 6 });
  ...
  test('builds schedule from new budget when previousBudget missing', () => {
    ...
    const schedule = service.buildCanaryScheduleData(fakeTestData, winningVariant, { canaryPct: 25, rampHours: 2 });
...
describe('autopilotService.applyAuto', () => {
  test('returns requires_approval when requiresApproval is true', async () => {
    await jest.isolateModulesAsync(async () => {
      ...
      variants: [
        { id: 'A', metrics: { views: 100, conversions: 10, engagement: 10 }, promotionSettings: { budget: 100 } },
        { id: 'B', metrics: { views: 120, conversions: 12, engagement: 12 }, promotionSettings: { budget: 100 } }
      ]
    };
    const doc = {
    ...
    const adminMock = { firestore: { FieldValue: { arrayUnion: () => {} } } };
      ...
      jest.doMock('../src/firebaseAdmin', () => ({ admin: adminMock, db: dbMock }), { virtual: true });
      jest.doMock('../src/abTestingService', () => ({ applyWinningSettings: async (contentId, variant) => ({ variantId: variant.id || variant.variantId || contentId }) }), { virtual: true });
      jest.doMock('../src/promotionService', () => ({ getContentPromotionSchedules: async () => [], updatePromotionSchedule: async () => {} }), { virtual: true });
      const service = require('../src/services/autopilotService');
  ...
  test('applies canary schedule when canaryPct is provided', async () => {
    await jest.isolateModulesAsync(async () => {
      ...
      const fakeTestData = {
        ...
        contentId: 'content-123',
        variants: [
          { id: 'A', metrics: { views: 1000, conversions: 40, engagement: 200, revenue: 400 }, promotionSettings: { budget: 200, platform: 'facebook' } },
          { id: 'B', metrics: { views: 200, conversions: 5, engagement: 50, revenue: 30 }, promotionSettings: { budget: 100 } }
        ],
        autopilotActions: []
      };
      let lastUpdate = null;
      const doc = {
        get: async () => ({ exists: true, data: () => fakeTestData }),
        update: async (u) => { lastUpdate = u; return true; }
      };
      const contentDoc = { exists: true, data: () => ({ optimizedPromotionSettings: { budget: 200 } }) };
      const dbMock = { collection: (name) => {
        if (name === 'ab_tests') return { doc: (id) => doc };
        if (name === 'content') return { doc: (id) => ({ get: async () => contentDoc, update: async () => true }) };
        return { doc: () => ({ get: async () => ({ exists:false, data: () => ({}) }) }) };
      } };
      const adminMock = { firestore: { FieldValue: { arrayUnion: (...args) => (args.length === 1 ? args[0] : args) } } };
      ...
      jest.doMock(abPath, () => ({ applyWinningSettings: async (contentId, variant) => ({ variantId: variant.id || variant.variantId || contentId }) }));
      let lastSchedData = null;
      jest.doMock(promoPath, () => ({ schedulePromotion: async (contentId, scheduleData) => { lastSchedData = scheduleData; return { id: 'sched-1', ...scheduleData }; }, getContentPromotionSchedules: async () => [], updatePromotionSchedule: async () => {}, deletePromotionSchedule: async () => ({ success: true }) }));
      ...
      const result = await service.applyAuto('test-123', { canaryPct: 10, rampHours: 12, promotionService: promo, abTestingService: absvc });
  ...
  test('rollbackAuto deletes created schedule and restores previous settings', async () => {
    await jest.isolateModulesAsync(async () => {
      ...
      const fakeTestData = {
        ...
        autopilotActions: [{ variantId: 'A', createdScheduleId: 'sched-1', previousPromotionSettings: { budget: 100 } }],
        winner: 'A',
        status: 'completed'
      };
      ...
      const doc = {
        get: async () => ({ exists: true, data: () => fakeTestData }),
        update: async (u) => { return true; }
      };
      const contentDoc = { exists: true, data: () => ({ optimizedPromotionSettings: { budget: 200 } }) };
      const dbMock = { collection: (name) => {
        if (name === 'ab_tests') return { doc: (id) => doc };
        if (name === 'content') return { doc: (id) => ({ get: async () => contentDoc, update: async (u) => { lastContentUpdate = u; return true; } }) };
        return { doc: () => ({ get: async () => ({ exists:false, data: () => ({}) }) }) };
      } };
      const adminMock = { firestore: { FieldValue: { arrayUnion: (...args) => (args.length === 1 ? args[0] : args) } } };
      jest.doMock('../src/firebaseAdmin', () => ({ admin: adminMock, db: dbMock }));
      const promoPath = require.resolve('../src/promotionService.js');
      jest.doMock(promoPath, () => ({ deletePromotionSchedule: async (id) => { deletedScheduleId = id; return { success: true }; }, getContentPromotionSchedules: async () => [{ id: 'future-1', start_time: new Date(Date.now() + 1000 * 60 * 60).toISOString(), budget: 20 }], updatePromotionSchedule: async (id, u) => { lastContentUpdate = lastContentUpdate || {}; lastContentUpdate.updated = u; return { id, ...u }; } }));
      const service = require('../src/services/autopilotService');
      const promo = require(promoPath);
