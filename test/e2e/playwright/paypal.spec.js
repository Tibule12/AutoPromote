...
test('upgrade flow opens PayPal and handles cancel return', async ({ page }) => {
  ...
  await page.route('**/api/paypal-subscriptions/plans', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ plans: [{ id: 'pro', name: 'Pro', price: 19.99 }, { id: 'free', name: 'Free', price: 0 }] }) });
  });
  ...
  await page.route('**/api/users/me', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ user: { email: 'test@local', uid: 'testUser', name: 'Test User' } }) }); });
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (url.includes('/api/paypal-subscriptions/create-subscription')) {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, approvalUrl: 'https://example.com/paypal-approve', subscriptionId: 'sub_123' }) });
      return;
    }
    if (url.includes('/api/paypal-subscriptions/plans')) {
      await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ plans: [{ id: 'pro', name: 'Pro', price: 19.99, features: { uploads: true } }, { id: 'free', name: 'Free', price: 0, features: {} }] }) });
      return;
    }
  ...
  const cardTitles = await page.evaluate(() => Array.from(document.querySelectorAll('.plans-grid .plan-card h4')).map(n => n.textContent.trim()));
  const subscribeButtons = await page.evaluate(() => Array.from(document.querySelectorAll('.plans-grid .plan-card')).map(card => ({ title: (card.querySelector('h4') && card.querySelector('h4').textContent.trim()) || '', hasSubscribe: !!card.querySelector('.subscribe-btn'), subscribeText: card.querySelector('.subscribe-btn') ? card.querySelector('.subscribe-btn').textContent.trim() : '' })));
...
test('activation flow after approval updates subscription state', async ({ page }) => {
  ...
  await page.route('**/api/paypal-subscriptions/status', async (route) => {
    await route.fulfill({
      status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, subscription: { planId: 'free', planName: 'Free', status: 'active', features: {} } })
    });
  });
