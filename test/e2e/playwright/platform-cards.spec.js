...
test('Per-platform card: Spotify preview, quality, upload', async ({ page }) => {
  ...
  await page.route('**/api/content/quality-check', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 88, quality_feedback: ['Looks good'] }) });
  });

  // Mock upload for preview (isDryRun true) and final upload
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    console.log('[ROUTE] intercept relative /api/content/upload', req.method(), body.substring ? body.substring(0, 400) : body);
    try {
      const json = JSON.parse(body || '{}');
      if (json.isDryRun) {
        const previews = [{ platform: 'spotify', thumbnail: '/default-thumb.png', title: json.title, description: json.description }];
        await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) });
        return;
      }
    } catch (e) {}
...
test('Per-platform card: Discord preview and upload', async ({ page }) => {
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 90 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try {
      const json = JSON.parse(body || '{}');
      if (json.isDryRun) {
        const previews = [{ platform: 'discord', thumbnail: '/default-thumb.png', title: json.title, description: json.description }];
        await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) });
        return;
      }
    } catch (e) {}
...
test('Per-platform card: Telegram preview and upload', async ({ page }) => {
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 85 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'telegram', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  const pageUrl = `file://${require('path').resolve(__dirname, '../fixtures/upload_component_test_page.html')}`;
  await page.goto(pageUrl);
...
test('Per-platform card: Reddit preview and upload', async ({ page }) => {
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 78 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'reddit', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  const pageUrl = `file://${require('path').resolve(__dirname, '../fixtures/upload_component_test_page.html')}`;
  await page.goto(pageUrl);
...
test('Per-platform card: LinkedIn preview and upload', async ({ page }) => {
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 92 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'linkedin', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  const pageUrl = `file://${require('path').resolve(__dirname, '../fixtures/upload_component_test_page.html')}`;
  await page.goto(pageUrl);
...
test('Per-platform card: Twitter preview and upload', async ({ page }) => {
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 69 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'twitter', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  const pageUrl = `file://${require('path').resolve(__dirname, '../fixtures/upload_component_test_page.html')}`;
  await page.goto(pageUrl);
...
test('Per-platform card: Snapchat preview and upload', async ({ page }) => {
  ...
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'snapchat', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  const pageUrl = `file://${require('path').resolve(__dirname, '../fixtures/upload_component_test_page.html')}`;
  await page.goto(pageUrl);
...
test('Per-platform card: Pinterest preview and upload', async ({ page }) => {
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 88 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'pinterest', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  const pageUrl = `file://${require('path').resolve(__dirname, '../fixtures/upload_component_test_page.html')}`;
  await page.goto(pageUrl);
...
test('Per-platform card: YouTube preview and upload', async ({ page }) => {
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 93 }) }); });
  let lastYouTubeUploadBody = null;
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'youtube', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    try {
      const json = JSON.parse(body || '{}');
      if (json && json.platform_options && json.platform_options.youtube) lastYouTubeUploadBody = json;
    } catch(_) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  const pageUrl = `file://${require('path').resolve(__dirname, '../fixtures/upload_component_test_page.html')}`;
  ...
  expect(lastYouTubeUploadBody.platform_options && lastYouTubeUploadBody.platform_options.youtube && lastYouTubeUploadBody.platform_options.youtube.visibility).toBe('public');
});
...
test('Per-platform SPA: Spotify preview & upload (dashboard)', async ({ page }) => {
  ...
  // Route mocks for SPA
  await page.route('**/api/content/quality-check', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 88 }) });
  });
  ...
  await page.route('**/api/content/upload', async (route, req) => {
    ...
    try {
      ...
      if (json && json.platform_options && json.platform_options.youtube) {
        console.log('[ROUTE] SPA YouTube payload visibility:', json.platform_options.youtube.visibility);
      }
    } catch (_) {}
    try {
      const json = JSON.parse(body || '{}');
  ...
  await page.route('https://firebasestorage.googleapis.com/**', async (route) => {
    ...
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ name: 'uploads/videos/fake.mp4', bucket: 'autopromote-cc6d3' }) });
  });
  // Intercept Firebase storage upload calls to avoid needing real credentials
  await page.route('https://firebasestorage.googleapis.com/**', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ name: 'uploads/videos/fake.mp4', bucket: 'autopromote-cc6d3' }) });
  ...
  await page.route('https://autopromote.onrender.com/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    console.log('[ROUTE] intercept absolute /api/content/upload', req.method(), body.substring ? body.substring(0, 400) : body);
    try {
      const json = JSON.parse(body || '{}');
      if (json.isDryRun) {
        const previews = [{ platform: 'spotify', thumbnail: '/default-thumb.png', title: json.title, description: json.description }];
  ...
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/content/my-promotion-schedules', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ schedules: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { spotify: { connected: true, meta: {} } } } ) }); });
  await page.route('**/api/health', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'OK' }) }); });
  await page.route('**/api/notifications', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ notifications: [] }) }); });
  await page.route('**/api/users/me', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ user: { email: 'test@local', name: 'Test User', uid: 'testUser' } }) }); });
  ...
  await page.waitForSelector('.platform-expanded .preview-button:not([disabled])', { timeout: 10000 });
  ...
  const contentInputFiles = await page.evaluate(() => (document.querySelector('#content-file-input') && document.querySelector('#content-file-input').files) ? document.querySelector('#content-file-input').files.length : 0);
  console.log('[debug] file input files count:', contentInputFiles);
  const spotifyTileClass = await page.evaluate(() => document.querySelector('div[aria-label="Spotify"]') ? document.querySelector('div[aria-label="Spotify"]').className : 'NOT FOUND');
  console.log('[debug] spotify tile class:', spotifyTileClass);
  const previewBtnDisabled = await page.evaluate(() => document.querySelector('.platform-expanded .preview-button') ? document.querySelector('.platform-expanded .preview-button').disabled : 'no-button');
  ...
  // Quality
  await Promise.all([
    page.waitForResponse(res => res.url().includes('/api/content/quality-check') && res.status() === 200),
  ...
  // Upload
  await page.waitForSelector('.platform-expanded .submit-button:not([disabled])', { timeout: 10000 });
  ...
  await page.waitForSelector('.platform-expanded .submit-button:not([disabled])', { timeout: 10000 });
  await page.locator('.platform-expanded button.submit-button').click();
  await page.waitForSelector('.platform-expanded .platform-upload-status:has-text("Upload")', { timeout: 50000 });
});

// Add YouTube SPA test
test('Per-platform SPA: YouTube preview & upload (dashboard)', async ({ page }) => {
  await page.setExtraHTTPHeaders({ 'x-playwright-e2e': '1' });
  await page.route('**/api/youtube/metadata', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ channel: { title: 'Test Channel', id: 'testchannel' } }) }); });
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 95 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'youtube', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  ...
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/content/my-promotion-schedules', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ schedules: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { youtube: { connected: true, meta: {} } } } ) }); });
  await page.route('**/api/health', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'OK' }) }); });
  await page.route('**/api/notifications', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ notifications: [] }) }); });
  await page.route('**/api/users/me', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ user: { email: 'test@local', name: 'Test User', uid: 'testUser' } }) }); });
  ...
  try { await page.waitForSelector('#youtube-visibility', { timeout: 10000 }); await page.selectOption('#youtube-visibility', 'public'); } catch(e) {}
  await Promise.all([
    page.waitForResponse(res => res.url().includes('/api/content/upload')),
    page.locator('.platform-expanded button.preview-button').click()
  ]);
...
test('Per-platform SPA: TikTok preview & upload (dashboard)', async ({ page }) => {
  ...
  await page.route('**/api/tiktok/creator_info', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ creator: { display_name: 'Test Creator', can_post: true, privacy_level_options: ['EVERYONE','FRIENDS','SELF_ONLY'], max_video_post_duration_sec: 60, interactions: { comments: true, duet: true, stitch: false } } }) });
  });
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 70 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'tiktok', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/content/my-promotion-schedules', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ schedules: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { tiktok: { connected: true, meta: {} } } } ) }); });
  await page.route('**/api/health', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'OK' }) }); });
  await page.route('**/api/notifications', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ notifications: [] }) }); });
  await page.route('**/api/users/me', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ user: { email: 'test@local', name: 'Test User', uid: 'testUser' } }) }); });

    // Bypass auth and set E2E flags (consent + bypass uploads) for SPA tests
  await page.addInitScript(() => { window.__E2E_BYPASS = true; window.__E2E_TEST_TOKEN = 'e2e-test-token'; window.__E2E_TEST_TIKTOK_CONSENT = true; window.__E2E_BYPASS_UPLOADS = true; localStorage.setItem('user', JSON.stringify({ uid: 'testUser', email: 'test@local', name: 'Test User', role: 'user' })); });
  await page.goto(BASE + '/#/dashboard', { waitUntil: 'networkidle' });
  await page.waitForSelector('nav li:has-text("Upload")', { timeout: 60000 });
  await page.click('nav li:has-text("Upload")');
  await page.waitForSelector('#content-file-input');
  ...
  const platformHtml = await page.evaluate(() => document.querySelector('.platform-expanded') ? document.querySelector('.platform-expanded').innerHTML : 'NO PLATFORM HTML');
  console.log('[DEBUG] platform-expanded HTML (short):', platformHtml.substring ? platformHtml.substring(0, 2000) : platformHtml);
  const labelsText = await page.evaluate(() => Array.from(document.querySelectorAll('.platform-expanded label')).map(l => l.textContent.trim()));
  ...
  const tkStatusText = await page.locator('.platform-expanded .platform-upload-status').textContent();
...
test('Per-platform SPA: Snapchat preview & upload (dashboard)', async ({ page }) => {
  ...
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/content/my-promotion-schedules', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ schedules: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { snapchat: { connected: true, meta: {} } } } ) }); });
  await page.route('**/api/health', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'OK' }) }); });
  await page.route('**/api/notifications', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ notifications: [] }) }); });
  await page.route('**/api/users/me', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ user: { email: 'test@local', name: 'Test User', uid: 'testUser' } }) }); });
  ...
  await page.locator('.platform-expanded button.submit-button').click();
  await page.waitForSelector('.platform-expanded .platform-upload-status');
  expect(await page.locator('.platform-expanded .platform-upload-status').textContent()).toMatch(/Upload|Publishing|submitted|Published/i);
});

test('Per-platform SPA: Pinterest preview & upload (dashboard)', async ({ page }) => {
  await page.setExtraHTTPHeaders({ 'x-playwright-e2e': '1' });
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 84 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'pinterest', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  ...
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/content/my-promotion-schedules', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ schedules: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { pinterest: { connected: true, meta: {} } } } ) }); });
  await page.route('**/api/health', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ status: 'OK' }) }); });
  await page.route('**/api/notifications', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ notifications: [] }) }); });
  await page.route('**/api/users/me', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ user: { email: 'test@local', name: 'Test User', uid: 'testUser' } }) }); });
  await page.addInitScript(() => { window.__E2E_BYPASS = true; window.__E2E_TEST_TOKEN = 'e2e-test-token'; window.__E2E_BYPASS_UPLOADS = true; localStorage.setItem('user', JSON.stringify({ uid: 'testUser', email: 'test@local', name: 'Test User', role: 'user' })); });
...
test('Per-platform SPA: Discord preview & upload (dashboard)', async ({ page }) => {
  ...
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { const previews = [{ platform: 'discord', thumbnail: '/default-thumb.png', title: json.title, description: json.description }]; await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { discord: { connected: true, meta: {} } } } ) }); });
  await page.addInitScript(() => { window.__E2E_BYPASS = true; window.__E2E_TEST_TOKEN = 'e2e-test-token'; window.__E2E_BYPASS_UPLOADS = true; localStorage.setItem('user', JSON.stringify({ uid: 'testUser', email: 'test@local', name: 'Test User', role: 'user' })); });
  await page.goto(BASE + '/#/dashboard', { waitUntil: 'networkidle' });
  await page.waitForSelector('nav li:has-text("Upload")', { timeout: 60000 });
  await page.click('nav li:has-text("Upload")');
  ...
  await page.waitForSelector('.platform-expanded');
  await page.waitForSelector('.platform-expanded input[placeholder="Discord channel ID"]', { timeout: 10000 });
...
test('Per-platform SPA: Telegram preview & upload (dashboard)', async ({ page }) => {
  ...
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { telegram: { connected: true, meta: {} } } } ) }); });
  await page.addInitScript(() => { window.__E2E_BYPASS = true; window.__E2E_TEST_TOKEN = 'e2e-test-token'; window.__E2E_BYPASS_UPLOADS = true; localStorage.setItem('user', JSON.stringify({ uid: 'testUser', email: 'test@local', name: 'Test User', role: 'user' })); });
  await page.goto(BASE + '/#/dashboard', { waitUntil: 'networkidle' });
  await page.waitForSelector('nav li:has-text("Upload")', { timeout: 60000 });
  await page.click('nav li:has-text("Upload")');
  ...
  await page.waitForSelector('.platform-expanded');
  await page.waitForSelector('.platform-expanded input[placeholder="Telegram chat ID"]', { timeout: 10000 });
...
test('Per-platform SPA: Reddit preview & upload (dashboard)', async ({ page }) => {
  ...
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { reddit: { connected: true, meta: {} } } } ) }); });
  await page.addInitScript(() => { window.__E2E_BYPASS = true; window.__E2E_TEST_TOKEN = 'e2e-test-token'; window.__E2E_BYPASS_UPLOADS = true; localStorage.setItem('user', JSON.stringify({ uid: 'testUser', email: 'test@local', name: 'Test User', role: 'user' })); });
  await page.goto(BASE + '/#/dashboard', { waitUntil: 'networkidle' });
  await page.waitForSelector('nav li:has-text("Upload")', { timeout: 60000 });
  await page.click('nav li:has-text("Upload")');
  ...
  await page.waitForSelector('.platform-expanded');
  await page.waitForSelector('.platform-expanded input[placeholder="Reddit subreddit"]', { timeout: 10000 });
...
test('Per-platform SPA: LinkedIn preview & upload (dashboard)', async ({ page }) => {
  ...
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { linkedin: { connected: true, meta: {} } } } ) }); });
  await page.addInitScript(() => { window.__E2E_BYPASS = true; window.__E2E_TEST_TOKEN = 'e2e-test-token'; window.__E2E_BYPASS_UPLOADS = true; localStorage.setItem('user', JSON.stringify({ uid: 'testUser', email: 'test@local', name: 'Test User', role: 'user' })); });
  await page.goto(BASE + '/#/dashboard', { waitUntil: 'networkidle' });
  await page.waitForSelector('nav li:has-text("Upload")', { timeout: 60000 });
  await page.click('nav li:has-text("Upload")');
  ...
  await page.waitForSelector('.platform-expanded');
  await page.waitForSelector('.platform-expanded input[placeholder="LinkedIn organization/company ID"]', { timeout: 10000 });
  await page.fill('.platform-expanded input[placeholder="LinkedIn organization/company ID"]', '98765');
  await page.setInputFiles('#content-file-input', 'test/e2e/playwright/test-assets/test.mp4');
  await page.locator('.platform-expanded button.preview-button').click();
  await page.waitForSelector('.platform-expanded .preview-card');
  await page.locator('.platform-expanded button.quality-check-button').click();
...
test('Per-platform SPA: Twitter preview & upload (dashboard)', async ({ page }) => {
  ...
  await page.route('**/api/content/my-content', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [] }) }); });
  await page.route('**/api/platform/status', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ raw: { twitter: { connected: true, meta: {} } } } ) }); });
  await page.addInitScript(() => { window.__E2E_BYPASS = true; window.__E2E_TEST_TOKEN = 'e2e-test-token'; window.__E2E_BYPASS_UPLOADS = true; localStorage.setItem('user', JSON.stringify({ uid: 'testUser', email: 'test@local', name: 'Test User', role: 'user' })); });
  await page.goto(BASE + '/#/dashboard', { waitUntil: 'networkidle' });
  await page.waitForSelector('nav li:has-text("Upload")', { timeout: 60000 });
  await page.click('nav li:has-text("Upload")');
  ...
  await page.waitForSelector('.platform-expanded');
  await page.waitForSelector('.platform-expanded input[placeholder="Twitter message (optional)"]', { timeout: 10000 });
  await page.fill('.platform-expanded input[placeholder="Twitter message (optional)"]', 'Test tweet');
  await page.setInputFiles('#content-file-input', 'test/e2e/playwright/test-assets/test.mp4');
  await page.locator('.platform-expanded button.preview-button').click();
  await page.waitForSelector('.platform-expanded .preview-card');
  await page.locator('.platform-expanded button.quality-check-button').click();
...
test('Per-platform card: TikTok respects creator_info and allows upload', async ({ page }) => {
  ...
  await page.route('**/api/content/quality-check', async (route) => { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ quality_score: 75 }) }); });
  await page.route('**/api/content/upload', async (route, req) => {
    const body = req.postData() || '';
    try { const json = JSON.parse(body || '{}'); if (json.isDryRun) { await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ previews: [{ platform: 'tiktok', thumbnail: '/default-thumb.png', title: json.title }] }) }); return; } } catch(e) {}
    await route.fulfill({ status: 201, contentType: 'application/json', body: JSON.stringify({ success: true }) });
  });
