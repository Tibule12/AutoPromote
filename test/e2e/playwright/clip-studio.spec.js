...
test('AI Clip Studio: analyze and generate clip (SPA)', async ({ page }) => {
  ...
  await page.route('**/api/content/my-content', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ content: [{ id: 'vid1', title: 'E2E Video', url: `${BASE}/test-assets/test.mp4`, duration: 60, type: 'video' }] }) });
  });
  ...
  await page.route('**/api/clips/analyze', async (route, req) => {
    ...
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, analysisId: 'a1', clipsGenerated: 1 }) });
  });

  await page.route('**/api/clips/analysis/a1', async (route) => {
    // Return a simple analysis with one topClip
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, analysis: { id: 'a1', duration: 60, scenesDetected: 4, transcriptLength: 1, topClips: [{ id: 'clip1', start: 0, end: 8, duration: 8, score: 90, reason: 'Great hook', platforms: ['tiktok'], captionSuggestion: 'Test caption', text: 'hello world' }] } }) });
  });
  ...
  await page.route('**/api/clips/generate', async (route, req) => {
    ...
    generatedClips.push({ id: 'gen-1', url: `${BASE}/test-assets/test.mp4`, viralScore: 90, duration: 8, caption: 'Test caption', platforms: ['tiktok'] });
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, clipId: 'gen-1' }) });
  });

  await page.route('**/api/clips/user', async (route) => {
    await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify({ success: true, clips: generatedClips, count: generatedClips.length }) });
  });
  ...
  await page.addInitScript(() => { window.__E2E_BYPASS = true; localStorage.setItem('user', JSON.stringify({ uid: 'testUser', email: 'test@local', name: 'Test User', role: 'user' })); });
  ...
  await page.waitForSelector('.clip-suggestion .btn-primary:has-text("Generate Clip")', { timeout: 10000 });
...
test.beforeAll(async () => {
  ...
  global.__clipServerProcess = spawn('node', ['test/e2e/playwright/static-server.js'], { stdio: 'inherit' });
