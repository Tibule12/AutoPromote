...
async function startServer() {
  ...
  fixturesApp.get("/__e2e/apiBaseUrl", (req, res) =>
    res.json({ apiBaseUrl: `http://127.0.0.1:${mainPort}` })
  );
...
async function runE2E() {
  ...
  try {
    ...
    await page.evaluate(() => {
      document.getElementById("description").value = "E2E test description";
    });
    await page.evaluate(() => {
      document.getElementById("type").value = "video";
    });
    await page.evaluate(() => {
      document.getElementById("url").value = "https://example.com/test-e2e.mp4";
    });
    // check spotify so we also test platform options pipeline
    await page.evaluate(() => {
      document.getElementById("target_spotify").checked = true;
    });
    // trigger submission
    ...
    try {
      ...
      console.warn(
        "Failed to cleanup content doc. You may need to remove it manually:",
        e && e.message
      );
    }
  } catch (err) {
    ...
    await new Promise((resolve, reject) =>
      mainServer ? mainServer.close(err => (err ? reject(err) : resolve())) : resolve()
    );
    await new Promise((resolve, reject) =>
      fixtureServer ? fixtureServer.close(err => (err ? reject(err) : resolve())) : resolve()
    );
  }
}
