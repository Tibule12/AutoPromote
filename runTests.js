...
try {
  ...
  if (process.env.GOOGLE_APPLICATION_CREDENTIALS) {
    try {
      sa = require(path.resolve(process.env.GOOGLE_APPLICATION_CREDENTIALS));
    } catch (e) {
      sa = null;
    }
  }
  ...
  if (!sa) {
    console.error(
      "❌ No service account available as env. Please set FIREBASE_ADMIN_SERVICE_ACCOUNT (JSON) or FIREBASE_ADMIN_SERVICE_ACCOUNT_BASE64 (base64) or GOOGLE_APPLICATION_CREDENTIALS pointing to a service account file. Do NOT use a committed service account JSON."
    );
    process.exit(1);
  }
  admin.initializeApp({
...
async function testRequiredCollections() {
  ...
  for (const collectionName of requiredCollections) {
    try {
      ...
      console.log(
        `✅ Collection '${collectionName}' exists and ${snapshot.empty ? "is empty" : "contains data"}`
      );
    } catch (error) {
      results[collectionName] = {
        exists: false,
...
async function runAllTests() {
  ...
  console.log(
    `Required Collections: ${Object.values(collectionsResult).every(r => r.exists) ? "✅ PASS" : "❌ FAIL"}`
  );
  console.log(`Dashboard Queries: ${queriesResult ? "✅ PASS" : "❌ FAIL"}`);

  const allPassed =
    connectionResult && Object.values(collectionsResult).every(r => r.exists) && queriesResult;
