...
const jsFiles = fs.readdirSync(jsDir).filter(file => file.endsWith('.js') && !file.endsWith('.LICENSE.txt'));
...
jsFiles.forEach(file => {
  ...
  githubPagesPatterns.forEach(pattern => {
    ...
    if (matches.length > 0) {
      ...
      content = content.replace(pattern, (match) => {
        return match.replace(/https:\/\/tibule12\.github\.io(?:\/AutoPromote)?(?:\/api\/)?/, `${backendUrl}/api/`);
      });
  ...
  const criticalEndpoints = [
    { pattern: /(['"])\/api\/auth\/register(['"])/g, replacement: `$1${backendUrl}/api/auth/register$2` },
    { pattern: /(['"])\/api\/auth\/login(['"])/g, replacement: `$1${backendUrl}/api/auth/login$2` },
    { pattern: /(['"])https:\/\/tibule12\.github\.io(?:\/AutoPromote)?\/api\/auth\/register(['"])/g, replacement: `$1${backendUrl}/api/auth/register$2` },
    { pattern: /(['"])https:\/\/tibule12\.github\.io(?:\/AutoPromote)?\/api\/auth\/login(['"])/g, replacement: `$1${backendUrl}/api/auth/login$2` }
  ];
  
  criticalEndpoints.forEach(endpoint => {
    const endpointMatches = content.match(endpoint.pattern) || [];
    if (endpointMatches.length > 0) {
      console.log(`  Found ${endpointMatches.length} instances of critical endpoint: ${endpoint.pattern}`);
