  ...
  const handleConfirm = async () => {
    ...
    try {
      const res = await fetch('/api/clips/generate-and-publish', {
        ...
        body: JSON.stringify({ contentId: contentItem.id, options: { aspect, addCaptions, platforms } })
      });
  ...
  return (
    <div className="gp-modal-overlay" role="dialog" aria-modal="true">
      <div className="gp-modal">
        ...
        <p className="gp-sub">Create a clip from <strong>{contentItem.title || contentItem.id}</strong> and publish to selected platforms.</p>
        ...
        <div className="gp-row gp-checkbox">
          <label>
            <input type="checkbox" checked={addCaptions} onChange={() => setAddCaptions(v => !v)} /> Add captions
          </label>
        </div>
        ...
        <div className="gp-row">
          ...
          <div className="gp-platforms">
            <label><input type="checkbox" checked={platforms.tiktok} onChange={() => togglePlatform('tiktok')} /> TikTok</label>
            <label><input type="checkbox" checked={platforms.youtube || false} onChange={() => togglePlatform('youtube')} /> YouTube</label>
          </div>
        </div>
        ...
        <div className="gp-actions">
          ...
          <button className="btn btn-primary" onClick={handleConfirm} disabled={status === 'in-progress' || status === 'queued'}>Confirm</button>
        </div>
      </div>
    </div>
  );
