...
const UserDashboard = ({ user, content, stats, badges, notifications, userDefaults, onSaveDefaults, onLogout, onUpload, mySchedules = [], onSchedulesChanged }) => {
  ...
  const withAuth = async (cb) => { const currentUser = auth?.currentUser; if (!currentUser) { alert('Please sign in first'); return; } const token = await currentUser.getIdToken(true); return cb(token); };
        ...
        {activeTab === 'schedules' && (
          <section className="schedules-panel">
            ...
            {schedulesList.length === 0 ? (
              <div style={{ color: '#9aa4b2' }}>No schedules yet. Create one by uploading content.</div>
            ) : (
              <div className="schedules-list" style={{ display: 'grid', gap: '.75rem', gridTemplateColumns: '1fr' }}>
                {schedulesList.map((sch, i) => (
                  <ScheduleCard key={i} schedule={sch} content={contentList.find(c => c.id === sch.contentId) || null} onPause={doPause} onResume={doResume} onReschedule={doReschedule} onDelete={doDelete} />
                ))}
              </div>
            )}
          </section>
