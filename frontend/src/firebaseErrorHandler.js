...
export const getFirebaseErrorMessage = error => {
  ...
  const errorCode =
    error.code ||
    (error.message && error.message.includes(":") ? error.message.split(":")[0].trim() : "unknown");
...
export const handleConnectionIssues = async () => {
  try {
    // TODO: implement reconnection logic (re-auth, network retry, etc.)
    // placeholder - currently assumes connection is fine
  } catch (error) {
