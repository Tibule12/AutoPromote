...
function AudioWaveformTrimmer({ file, trimStart, trimEnd, onChange }) {
  ...
  useEffect(() => {
    ...
    fetchAndDecode().catch(e => console.warn(e));
  }, [file]);
  useEffect(() => {
  ...
  function draw() {
    ...
    const sx = (trimStart / duration) * canvas.width || 0;
    const ex = (trimEnd / duration) * canvas.width || canvas.width;
    ctx.fillRect(0, 0, sx, canvas.height);
    ctx.fillRect(ex, 0, canvas.width - ex, canvas.height);
  }
  function setStart(e) {
    const x = e.target.value;
    const t = (x / 100) * duration;
    onChange && onChange({ trimStart: t, trimEnd });
  }
  function setEnd(e) {
    const x = e.target.value;
    const t = (x / 100) * duration;
    onChange && onChange({ trimStart, trimEnd: t });
  }
  return (
    <div style={{ display: "grid", gap: 8 }}>
      ...
      <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
        ...
        <input
          ...
          min="0"
          max="100"
          value={(trimStart / duration || 0) * 100}
