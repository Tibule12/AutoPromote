...
import { motionPose, motionPrimitives, normalizeMotion } from "./motionModel";
import "./motion.css";
...
export default function MotionCanvas({ scenes, getTime, time = 0 }) {
  ...
  const standaloneScene = scenes
    .map(normalizeMotion)
    .find(scene =>
      scene.canvasMode === "standalone" &&
      frame >= scene.startTime &&
      frame < scene.startTime + scene.duration
    );
  return (
    <svg
      ref={root}
    ...
    >
      {standaloneScene ? (
        <rect
          data-testid="motion-standalone-canvas"
          x="0"
          y="0"
          width="1000"
          height={h}
          fill={standaloneScene.canvasColor}
        />
      ) : null}
      {scenes.map(scene => {
        ...
        return (
          ...
          >
            {motionPrimitives(scene, pose).map((p, i) =>
              ...
              ) : p.kind === "stroke_circle" ? (
                <circle key={i} cx={p.x} cy={p.y} r={p.r} fill="none" stroke={p.fill} strokeWidth={p.width} />
              ) : p.kind === "line" ? (
                <line key={i} x1={p.x1} y1={p.y1} x2={p.x2} y2={p.y2} stroke={p.fill} strokeWidth={p.width} strokeLinecap="round" />
              ) : p.kind === "polygon" ? (
                <polygon key={i} points={p.points.map(point => point.join(",")).join(" ")} fill={p.fill} />
              ) : (
                <rect key={i} x={p.x} y={p.y} width={p.w} height={p.h} rx={p.radius || 0} fill={p.fill} />
              )
            )}
          </g>
        );
