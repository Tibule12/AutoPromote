...
The frontend exports `motionGraphics: {version: 1, scenes: [...]}` and the
...
forward both fields. The worker streams a transparent QTRLE/MOV overlay at the
programme dimensions and maps its frames back through the speed plan. It adds
that overlay and the sound cues to the composition before audio verification
and upload. Unsupported designs and failed motion/sound generation reject the
export instead of silently substituting or omitting the reviewed edit.
