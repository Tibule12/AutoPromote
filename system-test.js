...
async function testAdminFunctions() {
  ...
  try {
    ...
    if (adminToken) {
      ...
      const adminOverviewResponse = await apiRequest(
    ...
    if (contentId && userToken) {
      try {
        ...
        const analyticsResponse = await apiRequest(
          `/api/content/${contentId}/analytics`,
          "GET",
          null,
          userToken
        );
...
function outputResults() {
  ...
  if (testResults.overall === "Failed") {
    ...
    if (testResults.server.status !== "Passed") {
      console.log(
        "- Server is not responding correctly. Check if the server is running on port 5000."
      );
    }
    if (testResults.firebase.status !== "Passed") {
      console.log(
        "- Firebase connection is not working. Check serviceAccountKey.json and Firebase project settings."
      );
    }
    ...
    if (testResults.analytics.status !== "Passed") {
      console.log(
        "- Analytics functionality has issues. Check analytics routes and data structure."
      );
    }
  } else {
    console.log("\n✅ ALL TESTS PASSED");
