...
function wrapLines(src, max) {
  ...
  src.split(/\r?\n/).forEach(par => {
    ...
    par.split(/(\s+)/).forEach(tok => {
      if ((line + tok).length > max) {
        if (line.trim()) out.push(line.trimEnd());
        line = tok;
        if (tok.length > max) {
          while (tok.length > max) {
            out.push(tok.slice(0, max));
            tok = tok.slice(max);
          }
          line = tok;
        }
...
function buildPdf(text) {
  ...
  const lines = wrapLines(text, 95).map(escape);
  const lh = 14,
    top = 740,
    bottom = 40,
    usable = top - bottom,
    perPage = Math.floor(usable / lh);
  const pages = [];
  for (let p = 0; p * perPage < lines.length; p++) {
    const subset = lines.slice(p * perPage, p * perPage + perPage);
    let stream = "BT /F1 12 Tf 50 " + top + " Td ";
    subset.forEach((l, i) => {
      if (i === 0) stream += `(${l}) Tj`;
      else stream += ` 0 -${lh} Td (${l}) Tj`;
    });
    stream += " ET";
    pages.push(stream);
  }
  ...
  objects.push("@@PAGES@@");
  objects.push(obj(3, font));
  let nextId = 4;
  const contentIds = [];
  pages.forEach(s => {
    const body = `<< /Length ${Buffer.byteLength(s, "utf8")} >>\nstream\n${s}\nendstream`;
    objects.push(obj(nextId, body));
...
function main() {
  ...
  if (!fs.existsSync(src)) {
    console.error("Source clarification file missing:", src);
    process.exit(2);
  }
