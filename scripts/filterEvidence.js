...
function findLatestEvidence() {
  ...
  const files = fs
    .readdirSync(dir)
    .filter(f => /_evidence_.*\.json$/.test(f))
...
if (!file) {
  console.error(
    "No evidence file found in evidence/ and no file argument provided. Run the export script first."
  );
  process.exit(2);
}
...
try {
  ...
  const adminMatches = data.filter(e => {
    ...
    return (
      s.includes("admin") ||
      (e.type && /admin/i.test(e.type)) ||
      (e.eventType && /admin/i.test(e.eventType)) ||
      (e.action && /admin/i.test(e.action))
    );
  });
