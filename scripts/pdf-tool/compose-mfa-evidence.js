...
const imgs = [
  'policy_bundle.png',
  'host_inventory.png',
  'server_patch_log.png'
].map(f => path.join(imagesDir, f)).filter(p => fs.existsSync(p));
...
doc.fontSize(14).fillColor('black').text('MFA / Account Protection - Implementation Evidence', { align: 'left' });
doc.moveDown();
doc.fontSize(10).text('Cover note: This PDF contains implementation screenshots that demonstrate MFA/account protection enforcement or related policy pages. The reviewer should look for settings such as "Require MFA", "2FA enabled", or a 2-factor login prompt.', { align: 'left' });
doc.moveDown();
...
for (const img of imgs) {
  try {
    ...
    doc.image(img, { fit: [doc.page.width - 100, doc.page.height - 120], align: 'center', valign: 'center' });
    doc.moveDown();
    doc.fontSize(9).fillColor('gray').text('Source file: ' + path.basename(img), { align: 'left' });
  } catch (err) {
