...
function buildReport({ alertsReport, logExcerpt, sourceLog, alertsJsonPath }) {
  ...
  lines.push('Optional Slack integration via SECURITY_SLACK_WEBHOOK_URL for realtime notification.');
  lines.push('Incident response: verify user accounts targeted, lock compromised accounts, review IP reputation, escalate if persistent.');
  lines.push('');
...
function main() {
  ...
  const suspicious = accessLines.filter(l => /(status=401|status=403|status=5\d\d)/.test(l)).slice(0, 150);
  const reportText = buildReport({ alertsReport, logExcerpt: suspicious, sourceLog: path.basename(latestAccess), alertsJsonPath: path.basename(latestAlerts) });
