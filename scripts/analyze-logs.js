...
function analyze(entries) {
  ...
  for (const [k,v] of byBucket5xx.entries()) {
    if (v >= 10) { spikeBucket = { at: new Date(Number(k)).toISOString(), count: v }; break; }
  }
  if (rate > 1 || spikeBucket) {
    alerts.push({
      type: 'SERVER_ERRORS_SPIKE',
      severity: spikeBucket ? 'high' : 'medium',
      message: `5xx errors: ${fivexx}/${total} (${rate.toFixed(2)}%)` + (spikeBucket ? `; spike at ${spikeBucket.at} count=${spikeBucket.count}` : ''),
...
async function main() {
  ...
  if (!file || !fs.existsSync(file)) {
    console.error('No access log file found. Ensure LOG_EVENTS_TO_FILE=true and server has handled traffic.');
    process.exit(2);
  }
  ...
  const report = { generatedAt: new Date().toISOString(), source: path.basename(file), alerts, stats };
...
if (require.main === module) {
  main().catch(e => { console.error('Analyzer failed:', e.message); process.exit(1); });
}
