...
async function main() {
  if (
    ...
    console.warn(
      "No service account configured. Set GOOGLE_APPLICATION_CREDENTIALS or FIREBASE_ADMIN_SERVICE_ACCOUNT* to run cloud seed."
    );
    process.exit(1);
  }
  ...
  if (admin.apps.length === 0) {
    try {
      admin.initializeApp({
        ...
        projectId: process.env.FIREBASE_PROJECT_ID || process.env.GCLOUD_PROJECT || undefined,
      });
  ...
  console.log(
    ...
    process.env.FIREBASE_PROJECT_ID || process.env.GCLOUD_PROJECT || "default"
  );
  // Admin user
  await db
    .collection("admins")
    .doc("adminUser123")
    .set(
      { email: "admin@example.com", name: "Autopilot Admin", role: "admin", createdAt: now },
      { merge: true }
    );
  await db
    .collection("users")
  ...
  await contentRef.set(
    { title: "Test Content for Autopilot", userId: "testUser123", createdAt: now },
    { merge: true }
  );
  ...
  await db
    .collection("ab_test_variant_metrics")
    .doc("test-autopilot-1")
    .set(
      {
        variants: {
