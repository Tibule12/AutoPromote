...
function sampleEntries() {
  return [
    ...
    },
    {
      ...
      target: "user-5678",
      timestamp: iso(190),
      correlationId: "corr-1010",
      metadata: { role: "content_mod" },
    },
    {
      actorUid: "admin-redacted-2",
      action: "REVOKE_PRIVILEGE",
      outcome: "SUCCESS",
      ip: "198.51.100.77",
      target: "user-5678",
      timestamp: iso(170),
      correlationId: "corr-1015",
      metadata: { role: "content_mod" },
    },
    {
      actorUid: "admin-redacted-1",
      action: "EXPORT_DATA_INIT",
      outcome: "SUCCESS",
      ip: "203.0.113.24",
      target: "export-job-44",
      timestamp: iso(160),
      correlationId: "corr-1020",
      metadata: { scope: "user_report", rows: 245 },
    },
    {
      actorUid: "admin-redacted-2",
      action: "ATTEMPT_AUDIT_LOG_DELETE",
      outcome: "BLOCKED",
      ip: "198.51.100.77",
      target: "admin_audit_collection",
      timestamp: iso(155),
      correlationId: "corr-1025",
      metadata: { method: "DELETE" },
    },
    {
      actorUid: "admin-redacted-3",
      action: "ADMIN_LOGIN_ATTEMPT",
      outcome: "FAILURE",
      reason: "Invalid password",
      ip: "203.0.113.90",
...
function buildPdf(text) {
  ...
  for (let p = 0; p * per < lines.length; p++) {
    ...
    let stream = "BT /F1 " + fontSize + " Tf " + leftMargin + " " + top + " Td ";
    seg.forEach((l, i) => {
...
function main() {
  ...
  lines.push("JSON file: " + jsonName);
  lines.push("");
  lines.push(
    "Fields: actorUid, action, outcome, reason(if any), target, timestamp, ip, correlationId, metadata"
  );
  lines.push("");
  lines.push("Excerpt (first 10 events):");
  entries.forEach((e, i) => {
    lines.push(
      `${i + 1}. ${e.timestamp} actor=${e.actorUid} action=${e.action} outcome=${e.outcome}` +
  ...
  lines.push(
    "- Append-only storage; deletion attempts produce ATTEMPT_AUDIT_LOG_DELETE events (see sample)."
  );
  lines.push("- MFA required to access viewing interface; service account writes only.");
  lines.push(
    "- Weekly review + anomaly detection via analyzer correlating failed logins & tamper attempts."
  );
