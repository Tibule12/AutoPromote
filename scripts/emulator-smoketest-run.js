...
async function callPreview(){
  ...
  const res = await fetch(url, {
    method: 'GET',
    headers: { 'Authorization': 'Bearer test-token-for-adminUser', 'Content-Type':'application/json' }
  });
...
async function main(){
  ...
  try { await callPreview(); } catch(e){ console.log('Preview error', e.message); }
  try { await callSimulate(); } catch(e){ console.log('Simulate error', e.message); }
}
