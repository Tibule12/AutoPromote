...
async function run({ dry = false, limit = 20000 }) {
  ...
  snap.docs.forEach(d => {
    ...
    byContent[v.contentId] = byContent[v.contentId] || {
      clicksTotal: 0,
      variantClicks: {},
      variantStringClicks: {},
    };
    ...
    if (v.usedVariant) {
      bucket.variantStringClicks[v.usedVariant] =
        (bucket.variantStringClicks[v.usedVariant] || 0) + 1;
    }
    if (v.code) {
  ...
  console.log(
    "[backfill] content updated:",
    updates,
    "posts sampled shortlinks:",
    Object.keys(byShortlink).length
  );
}
