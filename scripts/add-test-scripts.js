...
if (!pkg.scripts["test:unit"]) {
  pkg.scripts["test:unit"] = "node test/basic-sanity.js";
  addedScripts.push("test:unit");
}
if (!pkg.scripts["test:routes"]) {
  pkg.scripts["test:routes"] = "node test/route-imports.test.js";
  addedScripts.push("test:routes");
}
if (!pkg.scripts["test"]) {
  pkg.scripts["test"] = "npm run test:unit";
  addedScripts.push("test");
}
