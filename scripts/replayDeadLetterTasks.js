...
(async function(){
  ...
  if (snap.empty) { console.log('No dead letter tasks.'); return; }
  let replayed=0; let skipped=0;
  for (const doc of snap.docs){
    const data = doc.data();
    // only replay if reason signature mismatch OR transient
    if (data.reason && !/signature/i.test(data.reason) && !/timeout|transient/i.test(data.reason)) { skipped++; continue; }
    const task = { ...data.originalTask, replayedAt: new Date().toISOString(), replaySource: 'replay_script' };
