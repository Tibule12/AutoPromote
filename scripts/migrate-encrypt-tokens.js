...
async function migrateUserConnections(limit=100) {
  ...
  let updates = 0; let scanned = 0; let changed = 0;
  for (const userDoc of usersSnap.docs) {
    ...
    try {
      ...
      for (const c of conns.docs) {
        ...
        if (data.tokens && typeof data.tokens === 'object' && !data.hasEncryption && hasEncryption()) {
          const tokenJson = JSON.stringify(data.tokens);
          console.log(`[migrate] user ${uid} connection ${c.id} — would encrypt tokens`);
          if (apply) {
            await c.ref.set({ tokens: encryptToken(tokenJson), hasEncryption: true }, { merge: true });
            updates++;
            changed++;
          }
        }
      }
    } catch (e) { console.warn('[migrate] user scan error', uid, e.message); }
  ...
  for (const doc of snap.docs) {
    ...
    if (data.access_token || data.refresh_token || data.accessToken || data.refreshToken) {
      ...
      if (apply && hasEncryption()) {
        await doc.ref.set({ tokenJson: encryptToken(JSON.stringify(candidate)), migratedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
        changed++;
      }
    }
  }
...
async function run() {
  try {
    console.log('[migrate] Dry-run mode:', !apply); console.log('[migrate] Using encryption key available:', !!hasEncryption());
    const cands = ['youtube_tokens','spotify_tokens','twitter_tokens','discord_tokens','pinterest_tokens','reddit_tokens','instagram_tokens','snapchat_tokens','linkedin_tokens','facebook_tokens','tiktok_tokens'];
  ...
  } catch (e) { console.error('[migrate] error', e && e.message, e); process.exit(1); }
}
