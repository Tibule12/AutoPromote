#!/usr/bin/env node
// Post a small diagnostic tweet using OAuth1 tokens stored in Firestore emulator
// Usage: FIRESTORE_EMULATOR_HOST=127.0.0.1:8080 node scripts/post-tweet-oauth1.js [uid]
...
async function main(){
  ...
  const statusText = process.env.DIAG_TWEET_TEXT || ('AutoPromote diagnostic: ' + new Date().toISOString());
  const url = 'https://api.twitter.com/1.1/statuses/update.json';

  const extraParams = { status: statusText };
  const authHeader = buildOauth1Header({ method: 'POST', url, consumerKey, consumerSecret, token: oauthToken, tokenSecret: oauthSecret, extraParams });

  const r = await fetch(url, { method: 'POST', headers: { Authorization: authHeader, 'Content-Type': 'application/x-www-form-urlencoded' }, body: new URLSearchParams(extraParams) });
