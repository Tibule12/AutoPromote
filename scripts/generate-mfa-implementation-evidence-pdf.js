...
function main() {
  ...
  let multiScanText =
    multiScanPath && fs.existsSync(multiScanPath)
      ? fs.readFileSync(multiScanPath, "utf8")
      : "Multi-port scan results missing (run remote-access-multiport-scan.ps1).";
  ...
  lines.push(
    "- GitHub: Settings -> Password and authentication -> 2FA enabled; organization requires MFA (screenshot: github_org_2fa.png)."
  );
  lines.push(
    "- Firebase Console: Logged-in admin email shows MFA enforced via Google Account (screenshot: firebase_google_mfa.png)."
  );
  lines.push(
    "- Any additional cloud provider accounts: confirm OTP/hardware key enforcement screenshot."
  );
  lines.push("");
  lines.push("Interpretation:");
  lines.push(
    "- Remote privileged service ports (SSH/RDP/DB/etc.) are blocked externally; only HTTPS ingress is exposed."
  );
  lines.push(
    "- SSH screenshot evidences connection refusal / timeout ensuring no direct shell access."
  );
  lines.push(
    "- Multi-port scan shows closed status for administrative ports, supporting remote access restriction."
  );
  lines.push(
    "- MFA screenshots (to be attached separately) validate multi-factor enforcement for privileged console/API access."
  );
  lines.push(
    "- Combined controls mitigate unauthorized data access and credential stuffing attacks."
  );
  lines.push("");
  ...
  lines.push(
    "- Weekly automated job should re-run multi-port scan and diff against prior results."
  );
