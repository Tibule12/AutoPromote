...
async function run() {
  try {
    ...
    const contentRef = await db
      .collection("content")
      .add({ title: "Autopilot Test Content", type: "video", created_at: new Date() });
    ...
    const variants = [
      {
        id: "variant-a",
        name: "A",
        promotionSettings: { platform: "youtube", budget: 50 },
        metrics: { views: 1000, conversions: 40 },
      },
      {
        id: "variant-b",
        name: "B",
        promotionSettings: { platform: "facebook", budget: 50 },
        metrics: { views: 1000, conversions: 10 },
      },
    ];
    ...
    await db
      ...
      .update({
        ...
        "autopilot.confidenceThreshold": 50,
        "autopilot.minSample": 10,
      });
    console.log("Autopilot enabled");
    // Simulate a metrics update which will invoke autopilot evaluation
    await abTestingService.updateTestMetrics(abTest.testId, "variant-a", {
      views: 1200,
      conversions: 50,
    });
