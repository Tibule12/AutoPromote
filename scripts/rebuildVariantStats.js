...
(async function main() {
  ...
  if (snap.empty) {
    console.log("No platform_posts found.");
    return;
  }
  ...
  for (const d of snap.docs) {
    ...
    if (v.usedVariant) {
      ...
      perPlat[v.usedVariant].clicks +=
        v.outcome && typeof v.outcome.clicks === "number" ? v.outcome.clicks : 0;
    }
  }
  let writes = 0;
  for (const [contentId, platMap] of grouped.entries()) {
    ...
    for (const [platform, variants] of Object.entries(platMap)) {
      doc.platforms[platform] = {
        variants: Object.entries(variants).map(([value, stats]) => ({
          value,
          posts: stats.posts,
          clicks: stats.clicks,
        })),
