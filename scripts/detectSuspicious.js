...
function isSuspicious(e) {
  try {
    ...
    // Heuristics:
    if (status && (status.includes('fail') || status.includes('unauth') || status.includes('error'))) return { reason: 'status indicates failure' };
    if (/admin/.test(type) && (status !== 'success')) return { reason: 'admin event with non-success status' };
    ...
    if (payload.includes('unauthorized') || payload.includes('invalid') || payload.includes('forbidden')) return { reason: 'contains unauthorized/invalid/forbidden' };
    if (payload.includes('token') && payload.includes('refresh')) return { reason: 'token refresh activity' };
    ...
    return null;
  } catch (err) { return { reason: 'error evaluating heuristics' }; }
}
...
(async ()=>{
  try {
    ...
    let query;
    try { query = db.collection(collectionName).orderBy('at', 'desc').limit(limit); }
    catch (_) {
      try { query = db.collection(collectionName).orderBy('timestamp', 'desc').limit(limit); }
      catch (_) { query = db.collection(collectionName).limit(limit); }
    }
    ...
    snap.forEach(doc => {
      ...
      if (s) {
        findings.push({ id: doc.id, doc: d, reason: s.reason, ts: ts ? ts.toISOString() : '(no-ts)' });
      }
    });
    ...
    const slackPayload = {
      ...
      findings: findings.map(f => ({ id: f.id, time: f.ts, reason: f.reason, type: f.doc.type || f.doc.eventType }))
    };
    ...
    if (findings.length === 0) console.log('No suspicious events found in last 7 days.');
      ...
      findings.forEach((f,i) => {
        console.log(`[${i+1}] id=${f.id} time=${f.ts} reason=${f.reason} type=${f.doc.type || f.doc.eventType || '(n/a)'} `);
      });
    }
    console.log('\nWrote alert files:\n', outTxt, '\n', outJson);
    process.exit(0);
