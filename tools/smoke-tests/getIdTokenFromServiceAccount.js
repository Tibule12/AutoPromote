...
async function main() {
  ...
  const apiKey = process.env.REACT_APP_FIREBASE_API_KEY || process.env.FIREBASE_API_KEY || process.env.FIREBASE_API_KEY_OVERRIDE || 'AIzaSyBA9It1gCyKBpqAhGM5TxwdNoe68c3qEBE';
  ...
  try {
    ...
    fs.writeFileSync(path.resolve(process.cwd(), 'tools', 'smoke-tests', '.idtoken'), data.idToken, { encoding: 'utf8' });
    process.exit(0);
  } catch (e) {
