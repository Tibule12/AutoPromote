...
async function listContent() {
  ...
  try {
    ...
    log(
      "LIST CONTENT",
      `${res.status} count=${json.content ? json.content.length : Array.isArray(json) ? json.length : "N/A"}`
    );
    return json;
  } catch (e) {
...
async function createContent() {
  if (!TOKEN) {
    log("CREATE", "Skipping: AUTH_TOKEN not provided");
    return null;
  }
...
async function getMyContent() {
  if (!TOKEN) {
    log("MY CONTENT", "Skipping: AUTH_TOKEN not provided");
    return null;
  }
...
async function enqueueYouTubeUpload(contentId) {
  if (!TOKEN) {
    log("ENQUEUE", "Skipping: AUTH_TOKEN not provided");
    return null;
  }
...
(async function main() {
  ...
  if (!healthOk)
    console.warn("Health check failed — proceed but results likely indicate server problems");
  ...
  if (TOKEN) {
    ...
    if (created && created.ok) {
      const cid =
        (created.body && created.body.content && created.body.content.id) ||
        (created.body && created.body.id) ||
        (created.content && created.content.id);
      if (cid) {
