...
const config = {
  publicClientId:
    (process.env.SNAPCHAT_PUBLIC_CLIENT_ID || process.env.SNAPCHAT_CLIENT_ID || "")
      .toString()
      .trim() || null,
  confidentialClientId:
    (process.env.SNAPCHAT_CONFIDENTIAL_CLIENT_ID || process.env.SNAPCHAT_CLIENT_ID || "")
      .toString()
      .trim() || null,
...
function ensureSnapchatEnv(res, cfg, opts = { requireSecret: true }) {
  ...
  if (!cfg.publicClientId && !cfg.confidentialClientId)
    missing.push("SNAPCHAT_PUBLIC_CLIENT_ID or SNAPCHAT_CLIENT_ID");
  ...
  if (missing.length > 0) {
    return res.status(500).json({
      ...
      missing: missing,
      message:
        "Please configure the required Snapchat environment variables. Note: Snapchat only supports production environment.",
    });
  }
}
...
router.get("/health", (req, res) => {
  try {
    ...
    return res.json({
      ok: !!(
        (cfg.publicClientId || cfg.confidentialClientId) &&
        (cfg.secret || process.env.ALLOW_NO_SECRET === "true") &&
        cfg.redirect
      ),
...
const oauthPrepareLimiter = rateLimiter({
  capacity: parseInt(process.env.SNAPCHAT_OAUTH_PREPARE_CAP || "60", 10),
  refillPerSec: 5,
  windowHint: "snapchat_oauth_prepare",
});
router.post("/oauth/prepare", authMiddleware, oauthPrepareLimiter, async (req, res) => {
  ...
  try {
    ...
    let scope = "snapchat-marketing-api";
    let scopeSource = "default";
    if (requestedScope && allowedScopes.has(String(requestedScope))) {
      scope = String(requestedScope);
      scopeSource = "request";
    } else if (envDefaultScope && allowedScopes.has(String(envDefaultScope))) {
      scope = String(envDefaultScope);
      scopeSource = "env";
    } else {
    ...
    try {
      ...
      const probe = await safeFetch(authUrl, fetch, {
        allowHosts: ["accounts.snapchat.com"],
        requireHttps: true,
        fetchOptions: { method: "GET", redirect: "manual" },
      });
      ...
      if (probe.status >= 500) {
        ...
        if (probe2.status < 500) {
          if (DEBUG_SNAPCHAT_OAUTH)
            console.log(
              "snapchat: primary auth URL returned",
              probe.status,
              "using fallback scope; probe2=",
              probe2.status
            );
          authUrl = fallbackUrl;
        } else {
          if (DEBUG_SNAPCHAT_OAUTH)
            console.warn(
              "snapchat: both primary and fallback auth URLs returned 5xx",
              probe.status,
              probe2.status
            );
    ...
    if (DEBUG_SNAPCHAT_OAUTH) {
      ...
      const mask = s => {
        try {
          if (!s) return null;
          const st = String(s);
          return st.length > 8 ? `${st.slice(0, 8)}…${st.slice(-4)}` : st;
        } catch (_) {
          return null;
        }
      };
    ...
    const scopeWarning =
      scope === "snapchat-marketing-api"
        ...
        : null;
    if (DEBUG_SNAPCHAT_OAUTH)
      console.log("snapchat: selected scope=%s source=%s", scope, scopeSource);
    res.json({
      authUrl,
      state,
      popup: popupRequested,
...
router.get(
  "/oauth/preflight",
  rateLimiter({ capacity: 60, refillPerSec: 10, windowHint: "snapchat_preflight" }),
  async (req, res) => {
    const cfg = activeConfig();
    ...
    try {
      ...
      let scope = "snapchat-marketing-api";
      let scopeSource = "default";
      const allowedS = ["snapchat-marketing-api", "display_name"];
      if (requestedQueryScope && allowedS.includes(requestedQueryScope)) {
        scope = requestedQueryScope;
        scopeSource = "request";
      } else if (envDefaultScope && allowedS.includes(envDefaultScope)) {
        scope = envDefaultScope;
        scopeSource = "env";
      }
      const clientIdForAuthorize = cfg.publicClientId || cfg.confidentialClientId;
...
const callbackLimiter = rateLimiter({
  capacity: parseInt(process.env.SNAPCHAT_CALLBACK_CAP || "300", 10),
  refillPerSec: 10,
  windowHint: "snapchat_callback",
});
router.all("/auth/callback", callbackLimiter, async (req, res) => {
  ...
  if (DEBUG_SNAPCHAT_OAUTH) {
    ...
    try {
      console.log(
        "snapchat: callback invoked method=%s remote=%s",
        req.method,
      ...
      console.warn("snapchat: callback debug log failed", e && e.message);
    }
  }
  if (!code) {
  ...
  try {
    ...
    if (DEBUG_SNAPCHAT_OAUTH)
      console.log(
        "snapchat: token exchange for state=%s user=%s scope=%s",
        state,
        userId,
        savedScope || null
      );
    ...
    if (req.method === "GET") {
      ...
      return res.redirect(
        `${DASHBOARD_URL}?snapchat=error&message=${encodeURIComponent(err.message || "oauth_failed")}`
      );
    }
...
router.get(
  ...
  authMiddleware,
  rateLimiter({
    capacity: parseInt(process.env.SNAPCHAT_STATUS_CAP || "300", 10),
    refillPerSec: 20,
    windowHint: "snapchat_status",
  }),
  async (req, res) => {
    try {
      const snap = await db
  ...
  async (req, res) => {
    try {
      ...
      const adAccountsPromises = organizations.map(async org => {
        try {
          const adAccountsRes = await safeFetch(
            `https://adsapi.snapchat.com/v1/organizations/${org.organization.id}/adaccounts`,
            fetch,
            {
              allowHosts: ["adsapi.snapchat.com"],
              requireHttps: true,
...
const apiActionLimiter = rateLimiter({
  capacity: parseInt(process.env.SNAPCHAT_API_ACTION_CAP || "120", 10),
  refillPerSec: 10,
  windowHint: "snapchat_api",
});
router.post("/creative", authMiddleware, apiActionLimiter, async (req, res) => {
  try {
    const { title, description, media_url, type, ad_account_id, call_to_action, web_url } =
      req.body;
    ...
    const creativeRes = await safeFetch(
    ...
    if (!creativeRes.ok) {
      const errorMsg =
        creativeData.request_status?.message || creativeData.error || "Unknown error";
...
router.get("/analytics/:creativeId", authMiddleware, apiActionLimiter, async (req, res) => {
  try {
    ...
    const queryParams = new URLSearchParams({
      fields:
        "impressions,swipes,quartile_1,quartile_2,quartile_3,view_completion,spend,conversion_purchases,conversion_save",
    ...
    const analyticsRes = await safeFetch(
      `https://adsapi.snapchat.com/v1/creatives/${creativeId}/stats?${queryParams.toString()}`,
      fetch,
      {
        allowHosts: ["adsapi.snapchat.com"],
        requireHttps: true,
    ...
    if (!analyticsRes.ok) {
      const errorMsg =
        analyticsData.request_status?.message || analyticsData.error || "Unknown error";
...
if (DEBUG_SNAPCHAT_OAUTH) {
  router.get("/_debug/authorize_probe", async (req, res) => {
    try {
      ...
      const r = await safeFetch(authUrl, fetch, {
        allowHosts: ["accounts.snapchat.com"],
        requireHttps: true,
