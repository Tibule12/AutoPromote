...
jest.mock("../firebaseAdmin", () => {
  ...
  const createMock = jest.fn(async obj => {
    savedAnalysis = obj;
    return;
  });
  const addMock = jest.fn(async obj => {
    savedAnalysisAdded = obj;
    return { id: "generated123" };
  });
  const db = {
    collection: name => {
      if (name === "content") {
  ...
  return {
    __mocks: {
      createMock,
      addMock,
      getSavedAnalysis: () => savedAnalysis,
      getSavedAnalysisAdded: () => savedAnalysisAdded,
    },
    db,
    storage: {
      bucket: () => ({
        upload: jest.fn(),
        file: () => ({ getSignedUrl: async () => ["https://signed.url"] }),
      }),
    },
  };
});
...
describe("VideoClippingService - analyze & generate integration", () => {
  afterEach(() => {
    ...
    // noop: reference to satisfy linters/coverage
    void fb.__mocks.getSavedAnalysis;
    // Clear mock state
    ...
    // reset saved values
    fb.__mocks.getSavedAnalysis &&
      typeof fb.__mocks.getSavedAnalysis === "function" &&
      (fb.__mocks.getSavedAnalysis._saved = null);
    fb.__mocks.getSavedAnalysisAdded &&
      typeof fb.__mocks.getSavedAnalysisAdded === "function" &&
      (fb.__mocks.getSavedAnalysisAdded._saved = null);
  });
  ...
  test("analyzeVideo saves topClips with id and captionSuggestion", async () => {
    ...
    jest
      .spyOn(svc, "generateTranscript")
      .mockResolvedValue([{ start: 0, end: 5, text: "hello world" }]);
