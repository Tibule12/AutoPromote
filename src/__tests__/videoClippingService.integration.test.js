  ...
  const db = {
    collection: (name) => {
          ...
          doc: (id) => ({
            get: async () => ({
              ...
              data: () => ({
                ...
                topClips: [{ id: 'clip-int-1', start: 0, end: 8, duration: 8, score: 90, platforms: ['tiktok'], captionSuggestion: 'Integ caption' }]
              })
            })
          })
        };
      ...
      if (name === 'generated_clips') {
        return { add: async (obj) => { savedGeneratedClip = obj; return { id: 'gen-1' }; } };
      }
...
(runIntegration && ffmpegAvailable() ? test : test.skip)('integration: generate clip end-to-end (requires ffmpeg)', async () => {
