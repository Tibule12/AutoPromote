    ...
    users.forEach(user => {
      ...
      if (user.lastActive) {
        const daysSinceActive =
          (Date.now() - (user.lastActive.toDate?.() || new Date(user.lastActive))) /
          (1000 * 60 * 60 * 24);
          ...
          if (platformData.variants && Array.isArray(platformData.variants)) {
            platformData.variants.forEach(variant => {
              tests.push({
                ...
                decayedCtr:
                  variant.decayedPosts > 0
                    ? (variant.decayedClicks / variant.decayedPosts) * 100
                    : 0,
...
router.get("/segments", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const byContentCreation = {
      ...
      regularCreators: Object.values(userContentCounts).filter(count => count >= 3 && count < 10)
        .length,
