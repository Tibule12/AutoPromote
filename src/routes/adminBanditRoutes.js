...
function normalizeWeights(w) {
  ...
  if ([ctr, reach, quality].some(v => typeof v !== "number" || isNaN(v) || v <= 0))
...
router.post("/rollback", authMiddleware, adminOnly, async (req, res) => {
  ...
  try {
    ...
    if (strategy === "custom") {
      if (!targetWeights)
        return res.status(400).json({ ok: false, error: "missing_target_weights" });
      ...
      if (strategy === "revision") {
        if (!targetRevisionAt)
          return res.status(400).json({ ok: false, error: "missing_targetRevisionAt" });
        ...
        for (const r of rows) {
          if (r.prev && r.next && !r.rollbackApplied && !r.rollback) {
            target = r.prev;
            break;
          }
        }
    ...
    try {
      await db.collection("bandit_weight_history").add(historyDoc);
    } catch (_) {}
    try {
      await db
        .collection("events")
        .add({
          type: "bandit_manual_rollback",
          at: historyDoc.at,
          reason,
          from: current,
          to: target,
    ...
    try {
      const { recordRollbackAlert } = require("../services/alertingService");
      recordRollbackAlert({ reason, manual: true });
    } catch (_) {}
