...
// Apply explicit CodeQL-detectable rate limiters per-route to satisfy scanners
...
router.post(
  "/:liveId/create-token",
  // limiter: authenticated write action (streamer creates tokens)
...
router.post(
  "/redeem",
  // public but should be limited to prevent abuse
...
router.get(
  "/validate",
  // validation is public but should have a general limiter
  codeqlLimiter && codeqlLimiter.general ? codeqlLimiter.general : (req, res, next) => next(),
  async (req, res) => {
    try {
