...
const platformPublicLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_PLATFORM_PUBLIC || "120", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "10"),
  windowHint: "platform_public",
});
const platformWriteLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_PLATFORM_WRITES || "60", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "5"),
  windowHint: "platform_writes",
});
const platformWebhookLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_PLATFORM_WEBHOOK || "300", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "50"),
  windowHint: "platform_webhook",
});
...
const TELEGRAM_WEBHOOK_WARN_THROTTLE_MS = parseInt(
  process.env.TELEGRAM_WEBHOOK_WARN_THROTTLE_MS || "300000",
  10
); // 5 minutes default
...
router.get(
  ...
  async (req, res) => {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ ok: false, platform, error: e && e.message ? e.message : "unknown_error" });
    }
  }
);
...
router.get(
  "/:platform/metadata",
  authMiddleware,
  rateLimit({ max: 20, windowMs: 60000, key: r => r.userId || r.ip }),
  async (req, res) => {
    const platform = normalize(req.params.platform);
    if (!SUPPORTED_PLATFORMS.includes(platform))
      return res.status(404).json({ ok: false, error: "unsupported_platform" });
    ...
    try {
          ...
          if (snap.exists) {
            ...
            if (tokens && tokens.access_token) {
              try {
                ...
                const r = await safeFetch(url, fetchFn, {
                ...
                if (r.ok) {
                  ...
                  result.meta.playlists = (j.items || []).map(p => ({
                    id: p.id,
                    name: p.name,
                    public: !!p.public,
                  }));
                  await userRef
                    ...
                    .set(
                      {
                        meta: { ...(sdata.meta || {}), playlists: result.meta.playlists },
      ...
      if (platform === "spotify") {
        ...
        try {
          ...
          if (r.ok) {
            ...
            await db
              ...
              .doc("spotify")
              .set(
                { meta: { playlists: result.meta.playlists }, updatedAt: new Date().toISOString() },
                { merge: true }
              );
          }
        } catch (e) {
        ...
        try {
          ...
          if (r.ok) {
            ...
            await db
              ...
              .doc("discord")
              .set(
                { meta: { guilds: result.meta.guilds }, updatedAt: new Date().toISOString() },
                { merge: true }
              );
          }
        } catch (e) {
        ...
        try {
          ...
          const r = await safeFetch(aclUrl, fetchFn, {
            fetchOptions: {
              headers: {
          ...
          if (r.ok) {
            const j = await r.json();
            const orgIds = (j.elements || [])
              .map(el => el && el.organizationalTarget && el.organizationalTarget.split(":").pop())
              .filter(Boolean);
            const orgs = [];
            for (const id of orgIds) {
              try {
                const orgReq = await safeFetch(
                  `https://api.linkedin.com/v2/organizations/${id}?projection=(localizedName)`,
                  fetchFn,
                  {
                    fetchOptions: {
                      headers: {
            ...
            await db
              ...
              .doc("linkedin")
              .set(
                { meta: { organizations: orgs }, updatedAt: new Date().toISOString() },
                { merge: true }
              );
          }
        } catch (e) {
              ...
              try {
                ...
                if (r.ok) {
                  ...
                  await userRef
                    ...
                    .set(
                      {
                        meta: { ...(sdata.meta || {}), boards: result.meta.boards },
                        updatedAt: new Date().toISOString(),
                      },
                      { merge: true }
                    );
...
router.post("/:platform/auth/simulate", authMiddleware, platformWriteLimiter, async (req, res) => {
  ...
  try {
    ...
    const fakeMeta = Object.assign(
      { display_name: `${platform} test user`, simulated: true },
      req.body.meta || {}
    );
    await userRef
      .collection("connections")
      .doc(platform)
      .set({ connected: true, meta: fakeMeta, simulated: true, updatedAt: now }, { merge: true });
    ...
    try {
      ...
      try {
        if (
          ...
          await userRef.set(
            { connectedPlatforms: admin.firestore.FieldValue.arrayUnion(platform) },
            { merge: true }
          );
        } else {
      ...
      try {
        const rec = smartDistributionEngine.calculateOptimalPostingTime(
          platform,
          /* timezone */ fakeMeta.timezone || "UTC"
        );
        const captionExample = engagementBoostingService.generateViralCaption(
          { title: "My first post", description: "" },
          platform,
          {}
        );
        await userRef
          ...
          .set(
            {
              recommendations: { posting: rec, captionExample },
              updatedAt: new Date().toISOString(),
            },
            { merge: true }
          );
  ...
  try {
    ...
    if (uid && uid !== "anon") {
      ...
      try {
        await db
          .collection("events")
          .add({ type: "platform_connected", uid, platform, at: new Date().toISOString() });
        try {
          const rec = smartDistributionEngine.calculateOptimalPostingTime(platform, "UTC");
          const captionExample = engagementBoostingService.generateViralCaption(
            { title: "Welcome post", description: "" },
            platform,
            {}
          );
          await userRef
          ...
          console.warn("[platform][reddit] recommendation generation failed", e && e.message);
        }
        try {
          if (
            admin &&
            ...
            const existing = (await userRef.get()).data() || {};
            const arr = Array.isArray(existing.connectedPlatforms)
    ...
    return sendPlain(
      res,
      500,
      "Reddit callback error: " + (e && e.message ? e.message : "unknown error")
    );
  }
});
...
router.get("/discord/auth/callback", platformPublicLimiter, async (req, res) => {
  ...
  try {
    ...
    const redirectUri =
      ...
      `${req.protocol}://${req.get("host")}/api/discord/auth/callback`;
    ...
    if (tokenJson.access_token) {
      const identityRes = await fetchFn("https://discord.com/api/users/@me", {
    ...
    if (uid && uid !== "anon") {
          ...
          if (aclRes.ok) {
            ...
            if (orgs.length > 0) {
              await userRef
                .collection("connections")
                .doc(platform)
                .set({ meta: { ...(meta || {}), organizations: orgs } }, { merge: true });
            }
          }
        }
      } catch (e) {
      ...
      try {
        if (tokenJson.access_token) {
          ...
          const gRes = await fetchFn(guildUrl, {
            headers: { Authorization: `Bearer ${tokenJson.access_token}` },
          });
          if (gRes.ok) {
            ...
            await userRef
              .collection("connections")
              .doc(platform)
              .set({ meta: { ...(meta || {}), guilds } }, { merge: true });
          }
        }
      } catch (e) {
        /* best-effort */
      ...
      try {
        if (tokenJson.access_token) {
          ...
          const pRes = await fetchFn(playlistsUrl, {
            headers: { Authorization: `Bearer ${tokenJson.access_token}` },
          });
          if (pRes.ok) {
            ...
            await userRef
              .collection("connections")
              .doc(platform)
              .set({ meta: { ...(meta || {}), playlists } }, { merge: true });
          }
        }
      } catch (e) {
        /* best-effort */
      ...
      try {
        ...
        try {
          ...
          console.warn("[platform][discord] recommendation generation failed", e && e.message);
        }
        try {
          if (
            admin &&
...
<p>Discord authorization complete. You can close this window.</p>
    ...
    try {
      const frontendBase =
        process.env.FRONTEND_URL || `${req.protocol}://${req.get("host").replace(/^api\./i, "")}`;
    ...
    return sendPlain(
      res,
      500,
      "Discord callback error: " + (e && e.message ? e.message : "unknown error")
    );
  }
});
...
router.get("/spotify/auth/callback", platformPublicLimiter, async (req, res) => {
  ...
  try {
    ...
    if (tokenJson.access_token) {
      try {
        const profileRes = await fetchFn("https://api.spotify.com/v1/me", {
          headers: { Authorization: `Bearer ${tokenJson.access_token}` },
        });
    ...
    if (uid && uid !== "anon") {
      ...
      try {
        ...
        try {
          ...
          console.warn("[platform][spotify] recommendation generation failed", e && e.message);
        }
        try {
          if (
            admin &&
    ...
    return sendPlain(
      res,
      500,
      "Spotify callback error: " + (e && e.message ? e.message : "unknown error")
    );
  }
});
...
router.get("/linkedin/auth/callback", platformPublicLimiter, async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.warn("[oauth][linkedin] state lookup failed", e && e.message);
    }
    if (!uid && state && state.split && state.split(":")[0]) uid = state.split(":")[0];
    if (uid && uid !== "anon") {
      const userRef = db.collection("users").doc(uid);
      ...
      try {
        ...
        try {
          ...
          console.warn("[platform][linkedin] recommendation generation failed", e && e.message);
        }
        try {
          if (
            admin &&
    ...
    return sendPlain(
      res,
      500,
      "LinkedIn callback error: " + (e && e.message ? e.message : "unknown error")
    );
  }
});
...
router.get("/pinterest/auth/callback", platformPublicLimiter, async (req, res) => {
  ...
  const oauthError = req.query.error;
  if (oauthError)
    return sendPlain(res, 400, `Pinterest error: ${req.query.error_description || oauthError}`);
  if (!code) {
    try {
      console.warn(
        "[oauth][pinterest] Missing code in callback; queryKeys=%s hostPresent=%s",
  ...
  try {
    ...
    if (tokenJson.access_token) {
      try {
        ...
        const accRes = await fetchFn("https://api.pinterest.com/v5/user_account", {
          headers: { Authorization: `Bearer ${tokenJson.access_token}` },
        });
        if (accRes.ok) meta.profile = await accRes.json();
        const boardsRes = await fetchFn("https://api.pinterest.com/v5/boards?limit=50", {
          headers: { Authorization: `Bearer ${tokenJson.access_token}` },
        });
        if (boardsRes.ok) {
    ...
    try {
      ...
      console.warn("[oauth][pinterest] state lookup failed", e && e.message);
    }
    if (!uid && state && state.split && state.split(":")[0]) uid = state.split(":")[0];
    if (uid && uid !== "anon") {
      const userRef = db.collection("users").doc(uid);
    ...
    return sendPlain(
      res,
      500,
      "Pinterest callback error: " + (e && e.message ? e.message : "unknown error")
    );
  }
});
...
router.post("/:platform/boards", authMiddleware, platformWriteLimiter, async (req, res) => {
  ...
  try {
    ...
    if (!uid) return res.status(401).json({ ok: false, error: "missing_user" });
    if (platform !== "pinterest")
      return res.status(400).json({ ok: false, error: "unsupported_platform_for_boards" });
    const body = req.body || {};
      ...
      try {
        ...
        try {
          ...
          if (!result.ok)
            return res
              .status(502)
              .json({ ok: false, error: result.error || "pinterest_api_error" });
          return res.json({ ok: true, board: result.board, simulated: result.simulated || false });
        } catch (e) {
          return res
            .status(500)
            .json({ ok: false, error: e && e.message ? e.message : "pinterest_create_failed" });
        }
      } catch (e) {
    ...
    try {
      ...
      if (!result.ok)
        return res
          .status(500)
          .json({ ok: false, error: result.error || "create_simulated_board_failed" });
      return res.json({ ok: true, board: result.board, simulated: result.simulated || false });
    } catch (e) {
      return res
        .status(500)
        .json({ ok: false, error: e && e.message ? e.message : "create_simulated_board_failed" });
    }
  } catch (e) {
  ...
  try {
    ...
    if (configuredSecret) {
      const incoming =
        ...
        req.get("x-telegram-bot-api-secret-token") ||
        req.get("x-telegram-secret-token");
      if (!incoming || String(incoming) !== String(configuredSecret)) {
        ...
        if (process.env.TELEGRAM_WEBHOOK_SILENT_REJECT === "true")
        ...
        try {
          ...
          if (now - last > TELEGRAM_WEBHOOK_WARN_THROTTLE_MS) {
            ...
            console.warn(
              "[telegram][webhook] invalid or missing secret token (throttled) ip=%s",
              remote
            );
      ...
      try {
        ...
        if (botToken) {
          await postToTelegram({
            payload: {
              text: "Thanks for contacting AutoPromote. Please connect your account from the app so we can link your Telegram.",
            },
            chatId,
          });
        }
    ...
    try {
      ...
      await userRef
        .collection("connections")
        .doc("telegram")
        .set({ connected: true, chatId, meta, updatedAt: now }, { merge: true });
      ...
      try {
        await db
          .collection("events")
          .add({ type: "platform_connected", uid, platform: "telegram", at: now });
      } catch (_) {}
      ...
      try {
        await postToTelegram({
          uid,
          payload: {
            text: "AutoPromote: your Telegram account is now connected. You will receive notifications here.",
          },
        });
      } catch (_) {}
    } catch (e) {
...
router.post("/spotify/playlists", authMiddleware, platformWriteLimiter, async (req, res) => {
  try {
    ...
    if (!result || !result.success)
      return res
        .status(502)
        .json({ ok: false, error: result.error || "spotify_create_playlist_failed" });
    return res.json({
      ok: true,
      playlist: { id: result.playlistId, name: result.name, url: result.url },
    });
  } catch (e) {
  ...
  async (req, res) => {
    try {
      ...
      if (!result || !result.success)
        return res
          .status(502)
          .json({ ok: false, error: result.error || "spotify_add_tracks_failed" });
