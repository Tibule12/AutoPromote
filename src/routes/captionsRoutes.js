...
router.post('/content/:id/captions', async (req,res) => {
  ...
  try {
    ...
    audit.log('captions.created', { userId, contentId: req.params.id, format: result.format, burnIn: !!burnIn });
...
router.get('/content/:id/captions/raw', async (req,res) => {
  ...
  try {
    ...
    const data = snap.data();
    if (data.user_id && data.user_id !== userId) return res.status(403).json({ error: 'forbidden' });
    if (!data.captions || !data.captions.assetId) return res.status(404).json({ error:'no_captions' });
    ...
    let body = null; let contentType = 'text/plain';
    if (fmtReq === 'vtt' && asset.vtt) { body = asset.vtt; contentType='text/vtt'; }
    else if (asset.srt) { body = asset.srt; contentType='application/x-subrip'; }
