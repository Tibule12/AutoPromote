  ...
  require("../statusInstrument")("aggregateStatus", async (req, res) => {
    ...
    const result = await dedupe(cacheKey, async () => {
      ...
      return {
        ...
        facebook:
          fb.connected === undefined
            ? fb
            : { connected: fb.connected, pages: fb.pages, identity: fb.identity },
        youtube: yt.connected === undefined ? yt : { connected: yt.connected, channel: yt.channel },
        twitter:
          tw.connected === undefined ? tw : { connected: tw.connected, identity: tw.identity },
        tiktok:
          tk.connected === undefined
            ? tk
            : { connected: tk.connected, display_name: tk.display_name },
        earnings: earnings.ok ? earnings : null,
        progress: progress.ok
          ? {
              contentCount: progress.contentCount,
              publishedCount: progress.publishedCount,
              promotionTasks: progress.promotionTasks,
