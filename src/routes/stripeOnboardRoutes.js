...
const stripePublicLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_STRIPE_PUBLIC || "120", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "10"),
  windowHint: "stripe_public",
});
const stripeWriteLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_STRIPE_WRITES || "30", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "3"),
  windowHint: "stripe_writes",
});
...
let rateLimit;
try {
  rateLimit = require("../middlewares/simpleRateLimit");
} catch (_) {
...
router.post(
  ...
  authMiddleware,
  stripeWriteLimiter,
  rateLimit({ max: 5, windowMs: 3600000, key: r => r.userId || r.ip }),
  async (req, res) => {
    try {
  ...
  try {
    ...
    if (!data.stripeAccountId)
      return res.json({
        ...
        onboarded: false,
        account: null,
        nextAction: "POST /api/stripe/onboard to start onboarding",
      });
    let account;
    try {
      account = await stripe.accounts.retrieve(data.stripeAccountId);
    } catch (e) {
      return res.status(400).json({ ok: false, error: "retrieve_failed", detail: e.message });
    }
    ...
    audit.log("stripe.account.status", {
      userId,
      accountId: account.id,
      chargesEnabled,
      payoutsEnabled,
    ...
    return res.json({
      ...
      nextAction:
        !chargesEnabled || !payoutsEnabled
          ? "Complete outstanding requirements in Stripe onboarding"
          : "Ready",
...
router.post(
  ...
  authMiddleware,
  stripeWriteLimiter,
  rateLimit({ max: 10, windowMs: 3600000, key: r => r.userId || r.ip }),
  async (req, res) => {
    try {
      if (!stripe) return res.status(400).json({ ok: false, error: "stripe_not_configured" });
      const userId = req.user.uid;
      ...
      if (!userDoc.exists || !userDoc.data().stripeAccountId)
        return res.status(404).json({ ok: false, error: "account_not_onboarded" });
