  ...
  try {
    ...
    if (!profileDoc.exists) {
      ...
      if (lastLogin !== today) {
        ...
        await db.collection('user_rewards').doc(userId).update({
...
router.get('/leaderboard', authMiddleware, async (req, res) => {
  try {
    ...
    let snapshot;
    if (type === 'points') {
      snapshot = await db.collection('user_rewards')
...
router.post('/referral', authMiddleware, async (req, res) => {
  try {
    ...
    await profileRef.set({
      ...
      totalPointsEarned: (profileDoc.data()?.totalPointsEarned || 0) + POINTS_CONFIG.referral_signup,
...
async function applyReward(userId, item) {
  switch (item.type) {
    ...
    case 'feature':
      ...
      await db.collection('user_features').doc(userId).set({
        [item.id]: {
          expiresAt: new Date(Date.now() + item.value.duration * 24 * 60 * 60 * 1000).toISOString(),
    ...
    case 'cosmetic':
      ...
      await db.collection('user_cosmetics').doc(userId).set({
        [item.id]: true,
        updatedAt: new Date().toISOString()
      }, { merge: true });
      break;
