...
const profilePublicLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_PROFILE_PUBLIC || "120", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "10"),
  windowHint: "profile_public",
});
const profileWriteLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_PROFILE_WRITES || "60", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "5"),
  windowHint: "profile_writes",
});

function validateDefaults(input) {
  const errors = [];
  if (input.timezone && typeof input.timezone !== "string") errors.push("timezone must be string");
  if (input.variantStrategy && !["rotation", "bandit"].includes(input.variantStrategy))
    errors.push("variantStrategy invalid");
  if (input.maxDailyUploads !== undefined) {
    if (
      typeof input.maxDailyUploads !== "number" ||
      input.maxDailyUploads < 1 ||
      input.maxDailyUploads > 1000
    )
      errors.push("maxDailyUploads out_of_range");
  }
...
router.get("/defaults", authMiddleware, profilePublicLimiter, async (req, res) => {
  try {
    ...
    if (process.env.TEST_OFFLINE === "true")
      return res.json({ ok: true, defaults: { mock: true } });
...
router.post("/defaults", authMiddleware, profileWriteLimiter, async (req, res) => {
  try {
    ...
    if (errs.length)
      return res.status(400).json({ ok: false, error: "validation_failed", details: errs });
    ...
    try {
      const { primeUserDefaults } = require("../services/userDefaultsCache");
      primeUserDefaults(req.userId, update);
    } catch (_) {}
    audit.log("profile.defaults.updated", {
      userId: req.userId,
      keys: Object.keys(update).filter(k => k !== "updatedAt"),
    });
...
router.post("/preview-schedule", authMiddleware, profilePublicLimiter, async (req, res) => {
  try {
    ...
    if (defaults.postingWindow && defaults.postingWindow.start) {
      ...
      const cand = new Date(
        Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate(), h || 15, m || 0, 0, 0)
      );
      if (cand < now) cand.setUTCDate(cand.getUTCDate() + 1);
      schedule = {
        when: cand.toISOString(),
        frequency: "once",
        timezone: defaults.postingWindow.timezone || defaults.timezone || "UTC",
      };
    }
