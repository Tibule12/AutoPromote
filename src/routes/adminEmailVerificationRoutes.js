...
const adminPublicLimiter = rateLimiter({
  ...
  windowHint: "admin_email_verification",
});
...
router.get("/email-verification/unverified", authMiddleware, requireAdmin, async (req, res) => {
  try {
    ...
    const unverified = list.users
      .filter(u => !u.emailVerified && !!u.email)
      .map(u => ({ uid: u.uid, email: u.email, created: u.metadata.creationTime }));
    res.json({
      ok: true,
    ...
    for (const u of candidates) {
      try {
        const link = await admin
          .auth()
          .generateEmailVerificationLink(u.email, {
        ...
        results.push({ email: u.email, uid: u.uid, status: "error", error: e.message });
      }
    }
