...
router.get("/errors", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const recentErrors = errors.filter(
      err => err.timestamp && new Date(err.timestamp) > last24Hours
    );
...
router.get("/api-metrics", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const avgResponseTime =
      requests.reduce((sum, r) => sum + (r.responseTime || 0), 0) / totalRequests || 0;
...
router.get("/query-performance", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    const avgQueryTime =
      queries.reduce((sum, q) => sum + (q.duration || 0), 0) / queries.length || 0;
