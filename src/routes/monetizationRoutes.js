...
const monetizationPublicLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_MONETIZATION_PUBLIC || "120", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "10"),
  windowHint: "monetization_public",
});
...
router.post("/referral/invite", authMiddleware, async (req, res) => {
  try {
    ...
    const invitation = await referralGrowthEngine.createReferralInvitation(userId, email, message);
...
router.post("/squad/share/:squadId", authMiddleware, async (req, res) => {
  try {
    ...
    const result = await referralGrowthEngine.shareWithGrowthSquad(userId, contentId, squadId);
