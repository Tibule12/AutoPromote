...
const variantAdminLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_VARIANT_ADMIN || "120", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "10"),
  windowHint: "variant_admin",
});
...
router.get("/strategy-stats", authMiddleware, adminOnly, variantAdminLimiter, async (_req, res) => {
  try {
    ...
    snap.forEach(d => {
      ...
      if (!v) counts.unspecified++;
      else if (String(v).toLowerCase() === "bandit") counts.bandit++;
      else counts.rotation++;
    });
  ...
  async (req, res) => {
    try {
      ...
      return res.json({
        ...
        exploration,
        total,
        ratio: total ? exploration / total : 0,
      });
    } catch (e) {
      return res.status(500).json({ ok: false, error: e.message });
    }
    ...
    try {
      ...
      try {
        ...
        if (vs && vs.platforms && vs.platforms[platform]) {
          profile.variants = vs.platforms[platform].variants.map(v => ({
            value: v.value,
            posts: v.posts,
            clicks: v.clicks,
            ...
            decayedCtr: v.decayedPosts ? v.decayedClicks / v.decayedPosts : null,
          }));
        }
      } catch (_) {}
