...
async function rollupContentDailyMetrics({ date } = {}) {
  const now = new Date();
  let target = date instanceof Date ? new Date(Date.UTC(date.getUTCFullYear(), date.getUTCMonth(), date.getUTCDate())) : null;
  if (!target) {
  ...
  postSnap.docs.forEach(d => {
    ...
    if (ts >= Date.parse(startIso) && ts < Date.parse(endIso)) {
      posts.push({ contentId: v.contentId, variant: v.usedVariant || null, impressions: v.metrics?.impressions || 0, clicks: v.clicks || 0 });
    }
  });
  const byContent = {};
  posts.forEach(p => {
    if (!p.contentId) return;
    if (!byContent[p.contentId]) byContent[p.contentId] = { posts: 0, impressions: 0, clicks: 0, variants: {}, variantClicks: {} };
  ...
  const resolveSnap = await db.collection('events')
  ...
  resolves.forEach(r => {
    ...
    if (!byContent[r.contentId]) byContent[r.contentId] = { posts: 0, impressions: 0, clicks: 0, variants: {}, variantClicks: {} };
