...
async function postToPinterest({ contentId, payload, reason, uid }) {
  ...
  const boardId = (payload && payload.boardId) || (payload && payload.platformOptions && payload.platformOptions.pinterest && payload.platformOptions.pinterest.boardId) || null;
  ...
  try {
    ...
    if (contentId && pinId && uid) {
      try {
        ...
        await contentRef.set({ pinterest: { pinId, boardId, note, postedAt: new Date().toISOString() } }, { merge: true });
      } catch (_) {}
    }
  ...
  try {
    ...
    if (hasAccessToken) {
      ...
      const r = await safeFetch('https://api.pinterest.com/v5/boards', fetchFn, {
        fetchOptions: { method: 'POST', headers: { Authorization: `Bearer ${accessToken}`, 'Content-Type': 'application/json' }, body: JSON.stringify(postBody) },
        requireHttps: true,
        allowHosts: ['api.pinterest.com']
      });
      const j = await (r.ok ? r.json() : r.text().then(t => ({ error: t })));
      if (!r.ok) return { ok: false, error: j.error || 'pinterest_api_error' };
      const board = { id: j.id || j.board_id || null, name: j.name || postBody.name, description: j.description || postBody.description || null };
      const existing = (conn.meta && Array.isArray(conn.meta.boards) ? conn.meta.boards : []);
      existing.push(board);
      await userRef.collection('connections').doc('pinterest').set({ meta: { ...(conn.meta || {}), boards: existing }, updatedAt: new Date().toISOString() }, { merge: true });
    ...
    const board = { id, name: String(name).trim(), description: description ? String(description).trim() : null };
    curMetaBoards.push(board);
    await userRef.collection('connections').doc('pinterest').set({ meta: { ...(conn.meta || {}), boards: curMetaBoards }, updatedAt: now, simulated: true }, { merge: true });
