...
if (process.env.USE_DISTRIBUTED_CACHE === 'true') {
  try { const { createDistributedCache } = require('./distributed/distributedCache'); distCache = createDistributedCache('user_defaults'); console.log('[cache] Distributed cache enabled for user_defaults'); } catch(e){ console.warn('[cache] Failed enabling distributed cache', e.message); }
}
...
async function fetchUserDefaults(userId) {
  ...
  if (distCache) {
    try {
      ...
      if (remote) { cache.set(userId, { data: remote, ts: now }); return remote; }
    } catch(_) { /* ignore */ }
  }
  try {
