...
let computeQualityScore; try { ({ computeQualityScore } = require('./variantQualityService')); } catch(_) { computeQualityScore = ()=>50; }
...
function heuristicGenerate({ title, need, existing }) {
  ...
  while(out.length<need && idx<patterns.length*3) {
    ...
    if (!existing.has(v)) { out.push(v); existing.add(v); }
    idx++;
  }
  return out;
}
...
function getGenerationStrategy(name) {
  switch((name||'heuristic').toLowerCase()) {
    case 'llm': return llmGenerateStub;
    default: return async (opts)=> heuristicGenerate(opts);
  }
}
