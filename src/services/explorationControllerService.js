...
async function adjustExplorationFactor() {
  ...
  const target =
    cfg.banditExplorationTarget ?? parseFloat(process.env.BANDIT_EXPLORATION_TARGET || "0.25");
  const tolerance = cfg.banditExplorationTolerance ?? 0.05; // ±5%
  const factor = cfg.banditExplorationFactor ?? 1.0;
  const { ratio } = await computeExplorationRatio({
    minutes: cfg.banditExplorationWindowMin || 180,
  });
  let newFactor = factor;
  if (ratio < target - tolerance) {
    newFactor = Math.min(factor * 1.1, 3.0); // increase up to 3x
  } else if (ratio > target + tolerance) {
    newFactor = Math.max(factor * 0.9, 0.2); // decrease lower bound
  }
  if (Math.abs(newFactor - factor) > 0.02) {
    await updateConfig({
      banditExplorationFactor: newFactor,
      banditExplorationLastAdjustment: new Date().toISOString(),
      banditExplorationObservedRatio: ratio,
    });
