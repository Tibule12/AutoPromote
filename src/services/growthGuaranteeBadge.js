...
function shouldAwardBadge(content) {
  ...
  return !!content.hashtags && Object.values(content.hashtags).some(tags => tags.includes('#AutoPromoteBoosted'));
}
...
function checkGrowthGuarantee(content, metrics) {
  ...
  const timeSincePromotion = (Date.now() - new Date(content.promotion_started_at).getTime()) / 3600000;
  if (timeSincePromotion >= 24 && metrics.views < threshold) {
    return { retryRequired: true, reason: `Views (${metrics.views}) below guarantee threshold (${threshold}) after 24h.` };
  }
...
function celebrateMilestone(content, metrics) {
  ...
  if (achieved.length) {
    return { milestone: achieved[achieved.length - 1], message: `🎉 Your content hit ${achieved[achieved.length - 1]} views!` };
  }
  return null;
}
