...
async function regenerateIfNeeded({ contentId, platform }) {
  ...
  const snap = await statsRef.get(); if(!snap.exists) return { skipped:true, reason:'no_stats' };
  const data = snap.data(); if (!data.platforms || !data.platforms[platform]) return { skipped:true, reason:'platform_absent' };
  ...
  const diversityLow = active.length < parseInt(process.env.VARIANT_MIN_ACTIVE || '2',10) && variants.length >= 1;
  ...
  const generated = await generateVariants({ title, targetCount: need, existingVariants: existingValues, strategy });
  if (!generated.length) return { skipped:true, reason:'no_unique_generated' };
  generated.forEach(g => {
    if (variants.some(v=> v.value === g.value)) return;
    variants.push({ value: g.value, posts:0, clicks:0, impressions:0, decayedClicks:0, decayedPosts:0, lastDecayAt: Date.now(), lastPostAt: null, anomaly:false, suppressed:false, quarantined:false, qualityScore:g.qualityScore });
  });
  const added = generated.length;
  await statsRef.set(data, { merge:true });
  try { await db.collection('events').add({ type:'variant_regenerated', contentId, platform, added, strategy, at: new Date().toISOString() }); } catch(_){ }
  return { added };
}
