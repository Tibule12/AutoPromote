...
async function executePayout(payoutDoc) {
  ...
  try {
    ...
    await db.collection('payouts').doc(payoutDoc.id).update({
      status: 'completed',
      externalBatchId: result && result.batch_header && result.batch_header.payout_batch_id || null,
    ...
    await db.collection('payouts').doc(payoutDoc.id).update({ status: 'failed', error: error && error.message, failedAt: new Date().toISOString() });
    return { success: false, error: error.message };
  }
}

async function processPendingPayouts(limit = 20) {
  const pendingSnap = await db.collection('payouts').where('status', '==', 'pending').limit(limit).get();
  if (pendingSnap.empty) return { processed: 0 };
  let processed = 0;
  for (const doc of pendingSnap.docs) {
    try {
      // mark processing
      await db.collection('payouts').doc(doc.id).update({ status: 'processing', processingAt: new Date().toISOString() });
