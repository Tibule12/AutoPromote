...
let cache = null; let cacheAt = 0; const TTL_MS = 60000; // 60s cache
...
async function getSettings() {
  ...
  try {
    ...
    cacheAt = Date.now();
  } catch(_) { cache = {}; cacheAt = Date.now(); }
  return cache;
}

function resolvePlatformCooldownHours(platform, settings) {
  return (settings.platformCooldownHours && settings.platformCooldownHours[platform])
