...
describe("platformPoster platformOptions merge", () => {
  ...
  test("dispatchPlatformPost merges platformOptions into top-level args for pinterest", async () => {
    ...
    expect(
      arg.boardId || arg.pinterest?.boardId || arg.payload?.platformOptions?.pinterest?.boardId
    ).toBeTruthy();
    expect(
      arg.boardId ||
        arg.boardid ||
        arg.pinterest?.boardId ||
        arg.payload?.platformOptions?.pinterest?.boardId
    ).toEqual("board123");
  });
});
