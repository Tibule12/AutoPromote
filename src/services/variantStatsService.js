...
async function updateVariantStats({
  ...
  await db.runTransaction(async tx => {
    ...
    const ctrs = arr
      .map(v => (v.decayedPosts > 0 ? v.decayedClicks / v.decayedPosts : 0))
    ...
    if (
      variantCtr > 0 &&
      medianCtr > 0 &&
      variantCtr >= medianCtr * ANOMALY_CTR_SPIKE_FACTOR &&
      row.decayedPosts >= ANOMALY_MIN_POSTS
    ) {
      row.anomaly = true;
      if (process.env.ENABLE_VARIANT_QUARANTINE === "true") {
    ...
    if (ENABLE_VARIANT_SUPPRESSION && !row.suppressed && row.posts >= SUPPRESSION_MIN_POSTS) {
      ...
      if (variantCtr < baseline * 0.6) {
        row.suppressed = true;
        row.suppressedAt = Date.now();
      }
    }
...
async function applyClickAttribution({ contentId, platform, variant, clicks = 1 }) {
  ...
  await db.runTransaction(async tx => {
    ...
    if (row.decayedPosts >= ANOMALY_MIN_POSTS && variantCtr > 0) {
      ...
      const ctrs = p.variants
        .map(v => (v.decayedPosts > 0 ? v.decayedClicks / v.decayedPosts : 0))
        .filter(n => !Number.isNaN(n))
        .sort((a, b) => a - b);
      const medianCtr = ctrs.length ? ctrs[Math.floor(ctrs.length / 2)] : 0;
      if (medianCtr > 0 && variantCtr >= medianCtr * ANOMALY_CTR_SPIKE_FACTOR) {
        row.anomaly = true;
...
async function addImpressions({ contentId, platform, variant, impressions }) {
  ...
  await db.runTransaction(async tx => {
    ...
    if (!plat) return;
    const row = plat.variants.find(v => v.value === variant);
    if (!row) return;
