...
function validateEnv({ strict = false } = {}) {
  ...
  CONDITIONALS.forEach(group => {
    if (group.condition()) {
      group.vars.forEach(v => { if (!process.env[v]) errors.push(`[${group.label}] missing ${v}`); });
    }
  });
  ...
  if (!process.env.SESSION_SECRET) warnings.push('SESSION_SECRET not set (recommended for future session-based flows)');
  if (!process.env.RATE_LIMIT_GLOBAL_MAX) warnings.push('RATE_LIMIT_GLOBAL_MAX not set (global limiter capacity)');
  if (process.env.ENABLE_DISTRIBUTED_LIMITER === 'true' && !process.env.REDIS_URL && !process.env.REDIS_HOST) {
    warnings.push('Distributed limiter enabled but no REDIS_URL or REDIS_HOST set');
  }
  if (process.env.DOC_SIGNING_SECRET && process.env.DOC_SIGNING_SECRET.includes('dev-doc-signing-secret')) {
  ...
  if (errors.length) {
    ...
    if (strict) {
      console.error(msg); errors.forEach(e => console.error(' -', e));
      process.exit(1);
    } else {
      console.warn(msg); errors.forEach(e => console.warn(' -', e));
    }
  }
  if (warnings.length) {
