...
class StartupDiagnostics {
  ...
  async checkFirebaseConnection() {
    ...
    try {
      // Test Storage
      const configuredBucket = process.env.FIREBASE_STORAGE_BUCKET || (admin && admin.options && admin.options.storageBucket);
      if (!configuredBucket) {
        this.log('warning', 'firebase', 'Firebase Storage bucket not configured', {
          action_required: 'Set FIREBASE_STORAGE_BUCKET in environment variables (e.g. my-bucket.appspot.com)',
        ...
        try {
          ...
          if (exists) {
            ...
            this.log('error', 'firebase', 'Firebase Storage bucket not found', {
              ...
              action_required: 'Create or configure storage bucket with this name, or set FIREBASE_STORAGE_BUCKET to the correct bucket',
          ...
          this.log('error', 'firebase', 'Firebase Storage connection failed', {
            ...
            bucket: configuredBucket,
            action_required: 'Verify service account has Storage permissions (roles/storage.objectViewer or roles/storage.admin) and that FIREBASE_SERVICE_ACCOUNT is valid',
            impact: 'File uploads will fail'
          });
        }
      }
  ...
  async saveToDatabase() {
    if (this.criticalErrors.length === 0 && this.errors.length === 0 && this.warnings.length === 0) {
      return;
    }
