...
function rateLimit(options = {}) {
  ...
  return async (req, res, next) => {
    try {
      ...
      if (
        bucket.count > (options.perMinute || parseInt(process.env.PER_MINUTE_LIMIT || "120", 10))
      ) {
        return res
          .status(429)
          .json({
            error: "rate_limited",
            scope: "minute",
            retryAfterMs: bucket.windowStart + IN_MEMORY_WINDOW_MS - now,
          });
      }
      ...
      if (existing + weight > dailyLimit) {
        ...
        return res
          .status(429)
          .json({ error: "rate_limited", scope: "day", limit: dailyLimit, resetAt });
      }
      await usageRef.set(
        { [field]: existing + weight, updatedAt: new Date().toISOString() },
        { merge: true }
      );
      next();
    } catch (e) {
