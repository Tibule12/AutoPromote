...
// POST /api/content/:id/thumbnail/extract-frames
// Extract candidate frames from a video for the user to pick from.
router.post(
  "/:id/thumbnail/extract-frames",
  authMiddleware,
  rateLimitMiddleware(10, 60000),
  async (req, res) => {
    try {
      ...
      const data = contentDoc.data() || {};
      if (data.user_id !== userId && !(req.user && req.user.isAdmin)) {
        return res.status(403).json({ error: "Forbidden" });
      }

      const mediaUrl =
        data.processedUrl || data.persistentMediaUrl || data.url || data.mediaUrl || data.video_url;
      if (!mediaUrl) return res.status(400).json({ error: "No video URL found for this content" });
      if (data.type !== "video") return res.status(400).json({ error: "Thumbnail extraction only works for videos" });

      const thumbnailService = require("./services/thumbnailService");
      const result = await thumbnailService.extractFrames(mediaUrl, {
        count: req.body.count || 8,
        strategy: req.body.strategy || "smart",
      });
      ...
      console.error("[thumbnail/extract-frames]", error);
      return res.status(500).json({ error: error.message || "Frame extraction failed" });
    }
  }
);

// POST /api/content/:id/thumbnail/generate
// Generate a finished thumbnail with text overlay for a specific platform.
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const mediaUrl =
        data.processedUrl || data.persistentMediaUrl || data.url || data.mediaUrl || data.video_url;
      if (!mediaUrl) return res.status(400).json({ error: "No video URL found" });
      ...
      const thumbnailService = require("./services/thumbnailService");
      const generated = await thumbnailService.generateThumbnail(mediaUrl, {
        time,
        platform,
        headline,
        subtitle,
        ...
        textColor,
        bgColor,
        showBrand: req.body.showBrand !== false,
      });

      let storageUrl = null;
      if (saveToStorage) {
        try {
          storageUrl = await thumbnailService.uploadThumbnailToStorage(
            userId,
            req.params.id,
            generated.dataUrl,
            platform
          );
          // Update the content document with the new thumbnail URL
          ...
          console.error("[thumbnail/generate] Storage upload failed", uploadErr);
        }
      }
...
// POST /api/content/:id/thumbnail/generate-all
// Generate platform-specific thumbnails for all requested platforms at once.
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const platforms = req.body.platforms || data.target_platforms || ["tiktok", "youtube", "instagram"];

      const thumbnailService = require("./services/thumbnailService");
      const result = await thumbnailService.generateAllPlatformThumbnails(mediaUrl, {
        platforms,
        time: req.body.time || 2,
        headline: req.body.headline || data.title || "",
        subtitle: req.body.subtitle || data.description || "",
      ...
      // Upload to storage for each platform
      const uploadPromises = Object.entries(result.thumbnails).map(([plat, thumb]) =>
        thumbnailService
          .uploadThumbnailToStorage(userId, req.params.id, thumb.dataUrl, plat)
          .then(url => ({ platform: plat, storageUrl: url }))
          .catch(err => ({ platform: plat, error: err.message }))
      );
      const uploads = await Promise.all(uploadPromises);

      // Save primary thumbnail to content doc
      const primaryPlatform = platforms[0] || "tiktok";
      const primaryUpload = uploads.find(u => u.platform === primaryPlatform && u.storageUrl);
      if (primaryUpload) {
        await db.collection("content").doc(req.params.id).update({
          thumbnailUrl: primaryUpload.storageUrl,
          thumbnails: uploads.filter(u => u.storageUrl).reduce((acc, u) => {
            acc[u.platform] = u.storageUrl;
            return acc;
          }, {}),
          thumbnailUpdatedAt: new Date().toISOString(),
        });
      ...
      return res.json({
        ok: true,
        thumbnails: Object.entries(result.thumbnails).reduce((acc, [plat, thumb]) => {
          const up = uploads.find(u => u.platform === plat);
          acc[plat] = {
            ...thumb,
            storageUrl: up?.storageUrl || null,
          };
          return acc;
        }, {}),
      });
    } catch (error) {
      console.error("[thumbnail/generate-all]", error);
      return res.status(500).json({ error: error.message || "Multi-platform thumbnail generation failed" });
    }
  }
);
