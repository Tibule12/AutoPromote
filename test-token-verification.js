...
if (!testToken) {
  console.error(
    "❌ No test token provided. Please add TEST_FIREBASE_TOKEN to your .env file or pass a token as an argument."
  );
...
auth
  ...
  .then(decodedToken => {
    ...
    if (timeToExpire < 0) {
      ...
      console.log(
        `⚠️ WARNING: Token will expire soon (in ${Math.round(timeToExpire / 60000)} minutes)`
      );
    } else {
  ...
  .catch(error => {
    ...
    switch (error.code) {
      ...
      case "auth/invalid-id-token":
        console.log(
          "📋 The token format is invalid. Check that you are sending a Firebase ID token."
        );
        break;
