...
exports.discordOAuthCallback = functions.region(region).https.onRequest(async (req, res) => {
  ...
  try {
    if (!(clientId && clientSecret && redirectUri)) {
      try { await admin.firestore().collection('oauth_states').doc(state || 'anon').set({ lastCallback: Date.now(), platform: 'discord', placeholder:true }, { merge: true }); } catch(_){}
      return res.status(200).send('Discord callback received; server missing client config for token exchange.');
    }
    ...
    if (uid && uid !== 'anon') {
      await admin.firestore().collection('users').doc(uid).collection('connections').doc('discord').set(storeData, { merge: true });
    } else {
      await admin.firestore().collection('discord_tokens').add({ tokenJson: encryptToken(JSON.stringify(tokenJson)), createdAt: Date.now() });
    }
    // Notify and close or redirect
    return res.status(200).send('Discord OAuth callback received. You can close this window.');
  } catch (e) {
    console.error('Discord callback error', e);
    return res.status(500).send('Discord callback error: ' + (e && e.message ? e.message : 'unknown'));
  }
});
