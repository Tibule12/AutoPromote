...
exports.instagramOAuthCallback = functions.region(region).https.onRequest(async (req, res) => {
  ...
  try {
    if (!(clientId && clientSecret && redirectUri)) {
      try {
        await admin
          ...
          .doc(state || "anon")
          .set(
            { lastCallback: Date.now(), platform: "instagram", placeholder: true },
            { merge: true }
          );
      } catch (_) {}
      return res
        .status(200)
        .send("Instagram callback received; server missing client config for token exchange.");
    }
    ...
    return res
      .status(500)
      .send("Instagram callback error: " + (e && e.message ? e.message : "unknown"));
  }
});
