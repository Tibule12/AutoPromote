...
exports.twitterOAuthCallback = functions.region(region).https.onRequest(async (req, res) => {
  ...
  try {
    if (!(clientId && clientSecret && redirectUri)) {
      try { await admin.firestore().collection('oauth_states').doc(state || 'anon').set({ lastCallback: Date.now(), platform: 'twitter', placeholder: true }, { merge: true }); } catch(_){}
      return res.status(200).send('Twitter callback received; server missing client config for token exchange.');
    }
    ...
    if (uid && uid !== 'anon') {
      await admin.firestore().collection('users').doc(uid).collection('connections').doc('twitter').set(storeData, { merge: true });
    } else {
      await admin.firestore().collection('twitter_tokens').add({ tokenJson: encryptToken(JSON.stringify(tokenJson)), createdAt: Date.now() });
    }
    ...
    return res.status(500).send('Twitter callback error: ' + (e && e.message ? e.message : 'unknown'));
  }
});
