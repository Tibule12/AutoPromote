...
exports.youtubeOAuthCallback = functions.region(region).https.onRequest(async (req, res) => {
  ...
  try {
    ...
    if (!tokenData.access_token) {
      return res
        .status(400)
        .json({ error: "Failed to obtain YouTube access token", details: tokenData });
    }
    ...
    res
      .status(200)
      .json({ success: true, channel: channelData.items ? channelData.items[0] : null });
  } catch (error) {
