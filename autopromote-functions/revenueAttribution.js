...
exports.logMonetizationEvent = functions.region(region).https.onCall(async (data, context) => {
  ...
  if (!contentId || !userId || !eventType) {
    throw new functions.https.HttpsError('invalid-argument', 'contentId, userId, and eventType are required');
  }
  try {
    ...
    const revenueField = (eventType === 'ad_click' || eventType === 'affiliate_conversion') ? 'revenue' : null;
    if (revenueField && value) {
      ...
      await admin.firestore().collection('revenue').doc(contentId).set({
        contentId,
        userId,
