...
exports.pinterestOAuthCallback = functions.region(region).https.onRequest(async (req, res) => {
  ...
  try {
    if (!(clientId && clientSecret && redirectUri)) {
      ...
      try {
        await admin
          ...
          .doc(state || "anon")
          .set(
            { lastCallback: Date.now(), platform: "pinterest", placeholder: true },
            { merge: true }
          );
      } catch (_) {}
      return res
        .status(200)
        .send("Pinterest callback received; server missing client config for token exchange.");
    }
    ...
    return res
      .status(500)
      .send("Pinterest callback error: " + (e && e.message ? e.message : "unknown"));
  }
});
