...
const TIKTOK_ENV =
  (process.env.TIKTOK_ENV || "sandbox").toLowerCase() === "production" ? "production" : "sandbox";
const sandboxConfig = {
...
exports.getTikTokAuthUrl = functions.region(region).https.onCall(async (data, context) => {
  if (!isTikTokConfigValid()) {
    throw new functions.https.HttpsError(
      "failed-precondition",
      `TikTok OAuth environment variables not set for mode ${TIKTOK_ENV}.`
    );
  }
...
exports.tiktokOAuthCallback = functions.region(region).https.onRequest(async (req, res) => {
  if (!isTikTokConfigValid()) {
    return res
      .status(500)
      .send(`TikTok OAuth environment variables not set for mode ${TIKTOK_ENV}.`);
  }
