...
async function getVariantDiversity(limitDocs = 150) {
  ...
  return {
    activeUnique,
    totalVariants: total,
    diversityRatio: total ? activeUnique / total : null,
  };
}
...
router.get("/overview", authMiddleware, adminOnly, async (req, res) => {
  try {
    ...
    if (weights.length >= 2) {
      ...
      if (curr && prev && curr.weights && prev.weights) {
        weightTrend = Object.keys(curr.weights).reduce((acc, k) => {
          acc[k] = {
            current: curr.weights[k],
            prev: prev.weights[k],
            delta: curr.weights[k] - prev.weights[k],
          };
          return acc;
        }, {});
      }
    ...
    const rollbacks = weights
      .filter(w => w.rollbackApplied || w.rollback)
      .map(w => ({
        at: w.at,
        reason: w.rollbackReason || w.reason || (w.rollback ? "auto" : "unknown"),
        restored: w.restored || w.prev || null,
      }));
    ...
    return res.json({
      ...
      exploration: explore,
      weightHistoryCount: weights.length,
      latestWeights: weights[0] || null,
      weightTrend,
      governance: gov,
      diversity,
      rollbacks,
...
router.get("/governance", authMiddleware, adminOnly, async (req, res) => {
  try {
    const gov = await getVariantGovernanceSummary();
    return res.json({ ok: true, ...gov });
  } catch (e) {
    return res.status(500).json({ ok: false, error: e.message });
  }
});
...
router.get("/diversity", authMiddleware, adminOnly, async (req, res) => {
  try {
    const d = await getVariantDiversity();
