...
const repostPublicLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_REPOST_PUBLIC || "120", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "10"),
  windowHint: "repost_public",
});
const repostWriteLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_REPOST_WRITES || "60", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "5"),
  windowHint: "repost_writes",
});
...
router.post("/actions/trigger/:contentId", authMiddleware, repostWriteLimiter, async (req, res) => {
  try {
    ...
    const actions = await repostDrivenEngine.triggerGrowthActions(contentId, repostMetrics);
...
router.post(
  "/markers/generate/:contentId/:platform",
  authMiddleware,
  repostWriteLimiter,
  async (req, res) => {
    try {
