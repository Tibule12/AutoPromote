...
const viralPublicLimiter = rateLimiter({
  ...
  windowHint: "viral_public",
});
const viralWriteLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_VIRAL_WRITES || "60", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "5"),
  windowHint: "viral_writes",
});
...
// POST /api/viral/check-bonus/:contentId - Trigger viral bonus check manually
router.post("/check-bonus/:contentId", authMiddleware, viralWriteLimiter, async (req, res) => {
  try {
...
router.post("/generate-hashtags", authMiddleware, viralWriteLimiter, async (req, res) => {
  try {
    ...
    await db.collection("hashtag_generations").add(
      cleanObject({
        userId,
...
router.post(
  ...
  authMiddleware,
  viralWriteLimiter,
  rateLimit({ max: 5, windowMs: 60000, key: r => r.userId || r.ip }),
  async (req, res) => {
    try {
      const userId = req.userId || req.user?.uid;
      if (!userId) {
      ...
      const viralPreview = {
        ...
        optimized: {
          title: algorithmOptimization.hook
            ? `${algorithmOptimization.hook} - ${content.title}`
            : content.title,
      ...
      const optimizationRef = await db.collection("content_optimizations").add(
        cleanObject({
          userId,
          contentId: content.id,
          platforms,
...
router.post("/track-repost", authMiddleware, viralWriteLimiter, async (req, res) => {
  try {
    ...
    const boostChainQuery = await db
      .collection("boost_chains")
    ...
    if (!boostChainQuery.empty) {
      ...
      const updatedChain = boostChainEngine.addBoostChainEvent(
        boostChain,
        userId,
        "manual_repost",
        {
...
router.get("/trending-sounds/:platform", authMiddleware, viralPublicLimiter, async (req, res) => {
  try {
    ...
    const trendingSounds = algorithmExploitationEngine.matchTrendingSound({ category }, platform);
...
router.get("/referral-stats", authMiddleware, viralPublicLimiter, async (req, res) => {
  try {
    ...
    const boostChainsQuery = await db
      .collection("boost_chains")
    ...
    const totalMembers = boostChains.reduce(
      (sum, chain) => sum + (chain.squadUserIds?.length || 0),
      0
    );
    const activeChains = boostChains.filter(chain => chain.status === "active").length;

    // Get user's content viral performance
    const userContentQuery = await db.collection("content").where("user_id", "==", userId).get();
      ...
      viralStats: {
        ...
        content_performance: {
          ...
          avg_views_per_content:
            viralContentCount > 0 ? Math.round(totalViralViews / viralContentCount) : 0,
        },
  ...
  async (req, res) => {
    try {
      ...
      if (squadId) {
        ...
        const squadRef = await db.collection("growth_squads").add(
          cleanObject({
            contentId,
            userIds: [userId],
            createdBy: userId,
