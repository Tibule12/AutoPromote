...
const abAdminLimiter = rateLimiter({ capacity: parseInt(process.env.RATE_LIMIT_AB_ADMIN || '60', 10), refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || '10'), windowHint: 'ab_admin' });
...
router.get('/:id/metrics', authMiddleware, adminOnly, abAdminLimiter, async (req, res) => {
  try {
    ...
    const postsSnap = await db.collection('platform_posts').where('contentId','==', contentId).orderBy('createdAt','desc').get().catch(()=>({ empty:true, docs:[] }));
    const buckets = {};
    const variantSet = new Set();
    postsSnap.docs.forEach(d => {
      const p = d.data();
      const created = p.createdAt && typeof p.createdAt.toDate === 'function' ? p.createdAt.toDate() : new Date(p.createdAt || Date.now());
      if (created < sinceDate) return; // off-window
      const dayKey = created.toISOString().split('T')[0];
      const variant = p.usedVariant || (p.rawOutcome && p.rawOutcome.usedVariant) || (typeof p.variantIndex === 'number' ? `variant_${p.variantIndex}` : 'unknown');
      ...
      buckets[dayKey][`${variant}_conversions`] = (buckets[dayKey][`${variant}_conversions`] || 0) + conversions;
    });
    ...
    const actions = (test.autopilotActions || []).slice().sort((a,b) => new Date(a.triggeredAt && a.triggeredAt.toDate ? a.triggeredAt.toDate() : a.triggeredAt || 0) - new Date(b.triggeredAt && b.triggeredAt.toDate ? b.triggeredAt.toDate() : b.triggeredAt || 0));
...
router.put('/:id/autopilot', authMiddleware, adminOnly, abAdminLimiter, async (req,res) => {
  try {
    const id = req.params.id;
    const { enabled, confidenceThreshold, minSample, mode, maxBudgetChangePercent, allowBudgetIncrease, requiresApproval } = req.body || {};
    if (!id) return res.status(400).json({ ok:false, error:'missing_test_id' });
    const updates = {};
    if (typeof enabled === 'boolean') updates['autopilot.enabled'] = enabled;
    if (typeof confidenceThreshold === 'number') updates['autopilot.confidenceThreshold'] = confidenceThreshold;
    if (typeof minSample === 'number') updates['autopilot.minSample'] = minSample;
    if (typeof mode === 'string') updates['autopilot.mode'] = mode;
    if (typeof maxBudgetChangePercent === 'number') updates['autopilot.maxBudgetChangePercent'] = maxBudgetChangePercent;
    if (typeof allowBudgetIncrease === 'boolean') updates['autopilot.allowBudgetIncrease'] = allowBudgetIncrease;
    if (typeof requiresApproval === 'boolean') updates['autopilot.requiresApproval'] = requiresApproval;
...
router.post('/:id/autopilot/preview', authMiddleware, adminOnly, abAdminLimiter, async (req,res) => {
  try {
    const id = req.params.id;
    if (!id) return res.status(400).json({ ok:false, error:'missing_test_id' });
    const snap = await db.collection('ab_tests').doc(id).get();
...
router.post('/:id/autopilot/simulate', authMiddleware, adminOnly, abAdminLimiter, async (req,res) => {
  try {
    const id = req.params.id;
    if (!id) return res.status(400).json({ ok:false, error: 'missing_test_id' });
    ...
    const { generatePosteriorSamplesForTopVsBaselineDeterministic } = require('../utils/statistics');
    const simSamples = generatePosteriorSamplesForTopVsBaselineDeterministic(test.variants || [], Number(samples), Number(seed));
    const sorted = (simSamples || []).slice().sort((a,b) => a - b);
    const percentile = (p) => { if (!sorted.length) return 0; const idx = Math.floor((sorted.length - 1) * p); return sorted[idx]; };
    ...
    if (typeof budgetPct !== 'undefined' && decision && decision.winner) {
      ...
      const currentBudget = (winning && winning.promotionSettings && typeof winning.promotionSettings.budget === 'number') ? winning.promotionSettings.budget : 0;
      const currentViews = (winning && winning.metrics && typeof winning.metrics.views === 'number') ? winning.metrics.views : 0;
      ...
      budgetSimulation = { pct, currentBudget, newBudget, currentViews, newViews, deltaViews, deltaConversions, deltaRevenue };
    }
    return res.json({ ok:true, decision, simulation: { samples: simSamples, p50, p95 }, budgetSimulation });
  } catch (e) { return res.status(500).json({ ok:false, error: e.message }); }
});
...
router.post('/:id/autopilot/rollback', authMiddleware, adminOnly, abAdminLimiter, async (req,res) => {
  try {
    const id = req.params.id;
    if (!id) return res.status(400).json({ ok:false, error:'missing_test_id' });
...
router.post('/:id/autopilot/approve', authMiddleware, adminOnly, abAdminLimiter, async (req,res) => {
  try {
    const id = req.params.id;
    if (!id) return res.status(400).json({ ok:false, error:'missing_test_id' });
    const user = req.user || {};
...
router.post('/:id/autopilot/unapprove', authMiddleware, adminOnly, abAdminLimiter, async (req,res) => {
  try {
    const id = req.params.id;
    if (!id) return res.status(400).json({ ok:false, error:'missing_test_id' });
    await db.collection('ab_tests').doc(id).update({
