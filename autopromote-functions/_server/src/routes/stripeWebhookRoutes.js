...
const stripeWebhookLimiter = rateLimiter({
  capacity: parseInt(process.env.RATE_LIMIT_STRIPE_WEBHOOK || "300", 10),
  refillPerSec: parseFloat(process.env.RATE_LIMIT_REFILL || "50"),
  windowHint: "stripe_webhook",
});
...
router.post(
  ...
  express.json({ limit: "2mb", verify: rawBodySaver }),
  async (req, res) => {
    ...
    try {
      try {
        const { applyStripeEvent } = require("../services/subscriptionSyncService");
