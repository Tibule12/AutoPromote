  ...
  try {
    ...
    if (idToken) {
      ...
      if (!firebaseApiKey) {
        ...
        return res.status(500).json({ error: "Server misconfiguration: password login unavailable" });
      }
      try {
        ...
        if (!authResp.ok || authJson.error) {
          console.warn("Email/password auth failed for email=%s code=%s", email, authJson.error?.message);
