...
function record(label, durMs, ok) {
  ...
  m.totalMs += durMs;
  if (!ok) m.errorCount++;
  if (durMs > m.max) m.max = durMs;
  // Keep sliding window (last 50 measurements) for rough p95
  m.p95Window.push(durMs);
  if (m.p95Window.length > 50) m.p95Window.shift();
...
async function instrument(label, fn) {
  ...
  try {
    ...
    record(label, Date.now() - start, ok);
  }
}
...
function getMetrics() {
  ...
  Object.keys(metrics).forEach(k => {
    const { count, errorCount, avg, p95, max } = metrics[k];
    out[k] = {
      count,
      errorCount,
      avg: Math.round(avg),
