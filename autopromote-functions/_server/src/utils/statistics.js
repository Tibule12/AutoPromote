...
function calculateBayesianConfidence(variants, samples = 4000) {
    ...
    const rates = variants.map(v => ({ id: v.id, conversions: v.metrics.conversions || 0, views: v.metrics.views || 0, rate: v.metrics.views ? (v.metrics.conversions || 0) / v.metrics.views : 0 }));
    let top = rates.reduce((p, c) => (c.rate > p.rate ? c : p), rates[0]);
    const others = rates.filter(r => r.id !== top.id);
    if (!others.length) return 0;
...
module.exports = { twoSampleProportionZTest, calculateConfidenceForVariants, calculateBayesianConfidence };
...
function generatePosteriorSamplesForTopVsBaseline(variants, samples = 400) {
    ...
    const rates = variants.map(v => ({ id: v.id, conversions: v.metrics.conversions || 0, views: v.metrics.views || 0 }));
    let top = rates.reduce((p, c) => (c.views && c.conversions / c.views > (p.views ? p.conversions / p.views : 0) ? c : p), rates[0]);
    const others = rates.filter(r => r.id !== top.id);
    if (!others.length) return [];
    const baseline = others.reduce((acc, r) => ({ conversions: acc.conversions + r.conversions, views: acc.views + r.views }), { conversions: 0, views: 0 });
    const out = [];
    for (let i = 0; i < samples; i++) {
        const topDraw = betaSample(top.conversions + 1, (top.views - top.conversions) + 1);
        const baseDraw = betaSample(baseline.conversions + 1, (baseline.views - baseline.conversions) + 1);
...
function generatePosteriorSamplesForTopVsBaselineDeterministic(variants, samples = 400, seed = 42) {
    ...
    for (let i = 0; i < samples; i++) {
        ...
        const baseDraw = betaSampleRNG(baseline.conversions + 1, (baseline.views - baseline.conversions) + 1, rng);
        out.push(topDraw - baseDraw);
    }
    return out;
}

module.exports.generatePosteriorSamplesForTopVsBaselineDeterministic = generatePosteriorSamplesForTopVsBaselineDeterministic;
