...
async function recentCtrAverage(minutes) {
  ...
  snap.docs.forEach(d => {
    const v = d.data();
    if (typeof v.rewardCtr === "number" && v.rewardCtr >= 0) {
      sum += v.rewardCtr;
      n++;
    }
  });
...
async function recordSelectionOutcome({
  contentId,
  platform,
  variant,
  rewardCtr,
...
async function computeSuggestedWeights() {
  ...
  snap.docs.forEach(d => {
    const v = d.data();
    const rc = v.rewardCtr || 0,
      rq = v.rewardQuality || 0,
      rr = v.rewardReach || 0;
    sumCtr += rc;
    sumQual += rq;
    sumReach += rr;
    n++;
    rewards.push({ rc, rq, rr });
  });
  if (!n) return null;
  // Basic averages
  ...
  try {
    ...
    if (cfg.rewardNormalization && cfg.rewardNormalization.method)
      method = cfg.rewardNormalization.method;
  } catch (_) {}
  let wCtrRaw = avgCtr,
    wQualRaw = avgQual,
    wReachRaw = avgReach;
  if (method === "zscore") {
    ...
    const ctrVals = rewards.map(r => r.rc);
    const qualVals = rewards.map(r => r.rq);
    const reachVals = rewards.map(r => r.rr);
    const ctrStd = std(ctrVals, avgCtr);
    const qualStd = std(qualVals, avgQual);
    const reachStd = std(reachVals, avgReach);
    wCtrRaw = avgCtr / ctrStd;
    wQualRaw = avgQual / qualStd;
    wReachRaw = avgReach / reachStd;
  }
  const total = wCtrRaw + wQualRaw + wReachRaw || 1;
  let wCtr = wCtrRaw / total,
    wQual = wQualRaw / total,
    wReach = wReachRaw / total;
  // Soft regularization: keep within [0.05,0.85]
  function clamp(v) {
    return Math.min(0.85, Math.max(0.05, v));
  }
  wCtr = clamp(wCtr);
  wQual = clamp(wQual);
  wReach = clamp(wReach);
  // Renormalize after clamp
  const renorm = wCtr + wQual + wReach;
  wCtr /= renorm;
  wQual /= renorm;
  wReach /= renorm;
  return {
...
async function applyAutoTune() {
  ...
  try {
    ...
    const newWeights = {
      ctr:
        (prev.ctr ?? parseFloat(process.env.BANDIT_WEIGHT_CTR || "0.6")) * (1 - LEARNING_RATE) +
        suggestion.wCtr * LEARNING_RATE,
      reach:
        (prev.reach ?? parseFloat(process.env.BANDIT_WEIGHT_REACH || "0.25")) *
          (1 - LEARNING_RATE) +
        suggestion.wReach * LEARNING_RATE,
      quality:
        (prev.quality ?? parseFloat(process.env.BANDIT_WEIGHT_QUALITY || "0.15")) *
          (1 - LEARNING_RATE) +
        suggestion.wQual * LEARNING_RATE,
    };
    const sum = newWeights.ctr + newWeights.reach + newWeights.quality || 1;
    newWeights.ctr /= sum;
    newWeights.reach /= sum;
    ...
    try {
      ...
      if (currentAvg != null && priorAvg != null && priorAvg > 0) {
        ...
        if (drop >= ROLLBACK_DROP_PCT) {
          ...
          try {
            await db
              .collection("bandit_weight_history")
              .add({ at: rollbackAt, rollback: true, dropPct: drop, restored: prev });
          } catch (_) {}
          try {
            const { recordRollbackAlert } = require("./alertingService");
            recordRollbackAlert({ reason: "ctr_drop", dropPct: drop, manual: false });
          } catch (_) {}
