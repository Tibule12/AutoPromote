...
class AutopilotService {
  ...
  buildCanaryScheduleData(testData, winningVariant, options = {}) {
    const prevBudget = (testData && testData.autopilot && testData.autopilot.previousPromotionSettings && testData.autopilot.previousPromotionSettings.budget) || 0;
    const newBudget = (winningVariant.promotionSettings && winningVariant.promotionSettings.budget) || prevBudget || 0;
    ...
    const canaryBudget = prevBudget > 0 ? Math.max(1, Math.round(prevBudget * (canaryPct / 100))) : Math.max(1, Math.round(newBudget * (canaryPct / 100)));
    const startTime = new Date();
    const endTime = new Date(startTime.getTime() + (rampHours * 60 * 60 * 1000));
    const schedule = {
      platform: (winningVariant.promotionSettings && winningVariant.promotionSettings.platform) || undefined,
  ...
  decideAutoApply(testData) {
    if (!testData || !testData.autopilot || !testData.autopilot.enabled) return { shouldApply: false, reason: 'autopilot_disabled' };
    ...
    if (totalViews < minSample) return { shouldApply: false, reason: 'min_sample_not_met', totalViews };
    ...
    const rates = testData.variants.map(v => ({ id: v.id, conversions: v.metrics.conversions || 0, views: v.metrics.views || 0, rate: v.metrics.views ? (v.metrics.conversions || 0) / v.metrics.views : 0, revenue: v.metrics.revenue || 0 }));
    const top = rates.reduce((p, c) => (c.rate > p.rate ? c : p), rates[0]);
    const others = rates.filter(r => r.id !== top.id);
    const baselineConversionsPerView = others.reduce((acc, r) => acc + (r.conversions / (r.views || 1)), 0) / (others.length || 1);
    const topRate = (top.conversions || 0) / (top.views || 1);
    const predictedUplift = baselineConversionsPerView ? ((topRate - baselineConversionsPerView) / baselineConversionsPerView) * 100 : 0;
    ...
    const avgRevenuePerConversion = top.conversions > 0 ? (top.revenue || 0) / (top.conversions || 1) : (others.reduce((acc, r) => acc + (r.conversions ? ((r.revenue || 0) / r.conversions) : 0), 0) / (others.length || 1));
    const estimatedRevenueChangePer1000Views = incConversionsPer1000Views * (avgRevenuePerConversion || 0);

    // risk score: derived from confidence but penalize small total views
    const sampleSafety = Math.max(0, Math.min(1, totalViews / Math.max(1, (testData.autopilot && testData.autopilot.minSample) || 100)));
    const riskScore = Math.max(0, Math.min(100, Math.round((1 - (confidence / 100) + (1 - sampleSafety)) * 100)));
    ...
    if (confidence >= threshold) {
      return { shouldApply: true, winner: winner.variantId, confidence, reason: 'above_threshold', predictedUplift, incConversionsPer1000Views, estimatedRevenueChangePer1000Views, baselineRate: baselineConversionsPerView, topRate, riskScore, simulation };
    }
    return { shouldApply: false, winner: winner.variantId, confidence, reason: 'below_threshold', predictedUplift, incConversionsPer1000Views, estimatedRevenueChangePer1000Views, baselineRate: baselineConversionsPerView, topRate, riskScore, simulation };
  }
  ...
  async applyAuto(testId, options = {}) {
    ...
    const variantScores = testData.variants.map(variant => ({ variantId: variant.id, score: this.calculateVariantScore(variant.metrics || {}), variant }));
    const winnerObj = variantScores.reduce((prev, curr) => curr.score > prev.score ? curr : prev, variantScores[0]);
    ...
    const newBudget = (winningVariant.promotionSettings && winningVariant.promotionSettings.budget) || prevBudget;
    const maxChange = (testData.autopilot && typeof testData.autopilot.maxBudgetChangePercent === 'number') ? testData.autopilot.maxBudgetChangePercent : 10;
    ...
    if (!allowIncrease && percentChange > 0) {
      ...
      return { applied: false, reason: 'budget_increase_disallowed', attemptedBudgetChangePercent: percentChange };
    }
    if (Math.abs(percentChange) > maxChange) {
      ...
      return { applied: false, reason: 'budget_change_exceeds_max', attemptedBudgetChangePercent: percentChange };
    }
    ...
    if (canaryPct && canaryPct > 0) {
      const promotionService = (options && options.promotionService) ? options.promotionService : require('../promotionService');
      const scheduleData = this.buildCanaryScheduleData(testData, winningVariant, { canaryPct, rampHours });
      try {
        createdSchedule = await promotionService.schedulePromotion(testData.contentId, scheduleData);
      } catch (e) {
    ...
    let winner = null;
    if (!createdSchedule) {
      const abTestingService = (options && options.abTestingService) ? options.abTestingService : require('../../abTestingService');
    ...
    await testRef.update({ status: 'completed', winner: winningVariantId, completedDate: new Date() });
  ...
  async rollbackAuto(testId, actionIndex = -1, options = {}) {
    ...
    if (createdScheduleId) {
      try {
        ...
        console.warn('Failed to delete canary schedule on rollback', createdScheduleId, e.message || e);
      }
    }
    if (previousPromotionSettings && testData.contentId) {
      await db.collection('content').doc(testData.contentId).update({ optimizedPromotionSettings: previousPromotionSettings });

      // Update any future promotions to the previous settings
      const futurePromotions = await promotionService.getContentPromotionSchedules(testData.contentId);
      for (const promotion of futurePromotions) {
        if (new Date(promotion.start_time) > new Date()) {
          await promotionService.updatePromotionSchedule(promotion.id, { ...promotion, ...previousPromotionSettings });
        }
      }
    }
