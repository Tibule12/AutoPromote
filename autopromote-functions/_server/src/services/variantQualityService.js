...
function computeQualityScore(message='') {
  ...
  if (len >= 40 && len <= 180) score += 8; else if (len > 180) score -= 5; else score -= 3;
  const hashtags = (text.match(/#[A-Za-z0-9_]+/g)||[]).length;
  if (hashtags === 1) score += 4; else if (hashtags === 2) score += 6; else if (hashtags >=3 && hashtags <=5) score += 8; else if (hashtags > 7) score -= 6;
  const emojis = (text.match(/\p{Extended_Pictographic}/u)||[]).length;
  if (emojis === 1) score += 3; else if (emojis >=2 && emojis <=5) score += 6; else if (emojis > 10) score -= 5;
  ...
  if (unique.size && words.length) {
    ...
    if (diversity > 0.7) score += 5; else if (diversity < 0.35) score -= 4;
  }
  // Clamp
