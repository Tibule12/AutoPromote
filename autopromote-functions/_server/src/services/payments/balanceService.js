...
async function computeUserBalance(userId) {
  ...
  let gross = 0;
  const earnings = []; // positive revenues
  ledgerSnap.docs.forEach(d => {
    const v = d.data();
    const ts = Date.parse(v.createdAt || "") || 0;
    if (ts >= sinceMs) {
      if (v.type === "subscription_fee" || v.type === "overage") {
        gross += v.amount || 0;
        earnings.push({ amount: v.amount || 0, ts });
      }
    }
  });
  ...
  payoutSnap.docs.forEach(d => {
    const v = d.data();
    if (["succeeded", "processing"].includes(v.status)) paid += v.amount || 0;
  });
  const provisional = Math.max(0, gross - paid);
  const cutoff = Date.now() - HOLD_DAYS * 86400000;
  let held = 0;
  earnings.forEach(e => {
    if (e.ts > cutoff) held += e.amount;
  });
