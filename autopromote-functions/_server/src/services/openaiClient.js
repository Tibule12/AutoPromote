...
const DEFAULT_OPENAI_BASE = process.env.OPENAI_API_BASE || ('https://' + ['api','openai','com'].join('.'));
...
async function audioTranscriptions(formData, meta = {}) {
  ...
  const headers = Object.assign({ Authorization: `Bearer ${OPENAI_API_KEY}` }, formData.getHeaders ? formData.getHeaders() : {});
