...
function repackageContent(content) {
  ...
  const newCaption = `${newTitle} ${Object.values(content.hashtags || {}).flat().join(' ')} #Remix #Retry`;
  const newHashtags = Object.fromEntries(Object.entries(content.hashtags || {}).map(([platform, tags]) => [platform, [...tags, '#Remix', '#Retry']]));
