...
class ContentAnalysisService {
  constructor() {
    ...
    // Assume we enable AI if key is present, otherwise fallback to mock/basic
    this.useAI = !!this.openaiApiKey;

    if (!this.openaiApiKey) {
      logger.warn("[ContentAnalysis] ⚠️ OPENAI_API_KEY not set. Using basic heuristic analysis.");
    }
  }
...
Provide analysis in JSON format with:
{
  ...
  "postingStrategy": "best time and frequency advice"
      ...
      // MITIGATION STRATEGY: Use cheaper model for bulk processing
      // gpt-4o-mini is ~95% cheaper than gpt-4o and perfect for JSON classification tasks
      const model = process.env.OPENAI_MODEL_GPT40_MINI || "gpt-4o-mini";

      const aiResp = await chatCompletions(
        {
          model: model,
          messages: [
            { role: "system", content: "You are a viral content strategist. Provide JSON output." },
...
// Export both the class (for testing/mocking) and a singleton instance
module.exports = {
