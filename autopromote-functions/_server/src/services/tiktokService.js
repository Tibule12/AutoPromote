 ...
 */
function generateAuthUrl({ clientKey, redirectUri, state, scope = 'user.info.basic,video.upload,video.publish' }) {
...
async function initializeVideoUpload({ accessToken, videoSize, chunkSize = 10485760 }) {
  ...
  const response = await safeFetch('https://open.tiktokapis.com/v2/post/publish/video/init/', fetchFn, {
    fetchOptions: {
      method: 'POST',
      headers: {
...
async function uploadVideoChunk({ uploadUrl, videoBuffer, chunkIndex, totalChunks }) {
  ...
  const response = await fetch(uploadUrl, {
    ...
    body: videoBuffer.slice(chunkIndex * 10485760, Math.min((chunkIndex + 1) * 10485760, videoBuffer.length))
  });
 ...
 */
async function publishVideo({ accessToken, publishId, title, privacyLevel = 'PUBLIC_TO_EVERYONE' }) {
  if (!fetchFn) throw new Error('Fetch not available');
  ...
  const response = await safeFetch(`https://open.tiktokapis.com/v2/post/publish/status/fetch/?publish_id=${publishId}`, fetchFn, {
    fetchOptions: {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${accessToken}`,
