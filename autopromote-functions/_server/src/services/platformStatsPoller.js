...
const { fetchFacebookPostMetrics, fetchTwitterTweetMetrics, fetchInstagramMediaMetrics } = require('./platformMetricsService');
...
async function fetchMetricsForPost(doc) {
  ...
  try {
    switch(platform) {
      case 'facebook': return await fetchFacebookPostMetrics(externalId);
      case 'twitter': return await fetchTwitterTweetMetrics(externalId);
      case 'instagram': return await fetchInstagramMediaMetrics(externalId);
      default: return null; // tik tok placeholder
    }
  } catch (_) { return null; }
}
...
async function pollPlatformPostMetricsBatch({ batchSize = 5, maxAgeMinutes = 30 } = {}) {
  ...
  for (const doc of snap.docs) {
    ...
    const lastCheckMs = d.lastMetricsCheck && d.lastMetricsCheck.toMillis ? d.lastMetricsCheck.toMillis() : 0;
      ...
      if (metrics) {
        ...
        try {
          ...
          if (lastEntry) {
            ...
            // Update peak
            const peak = typeof d.peakScore === 'number' ? Math.max(d.peakScore, normalizedScore) : normalizedScore;
            ...
            if (!d.accelTriggeredAt && acceleration >= accelThreshold) {
              ...
              await db.collection('analytics').add({
                type: 'platform_score_acceleration', platform: d.platform, contentId: d.contentId, postId: d.externalId, acceleration, normalizedScore, threshold: accelThreshold, createdAt: admin.firestore.FieldValue.serverTimestamp() });
              try { await recordPlatformAccelerationTrigger(); } catch(_){ }
            }
            // Decay detection
            ...
            if (!d.decayedAt && peak > 0 && normalizedScore < peak * (1 - decayPct)) {
              ...
              await db.collection('analytics').add({ type: 'platform_score_decay', platform: d.platform, contentId: d.contentId, postId: d.externalId, normalizedScore, peak, decayPct, createdAt: admin.firestore.FieldValue.serverTimestamp() });
              try { await recordPlatformDecayEvent(); } catch(_){ }
            }
            // Reactivation: after decay, climbs above (peak * (1 - decayPct/2))
            if (d.decayedAt && !d.reactivatedAt && peak > 0 && normalizedScore >= peak * (1 - decayPct / 2)) {
              update.reactivatedAt = admin.firestore.FieldValue.serverTimestamp();
              await db.collection('analytics').add({ type: 'platform_score_reactivation', platform: d.platform, contentId: d.contentId, postId: d.externalId, normalizedScore, peak, createdAt: admin.firestore.FieldValue.serverTimestamp() });
              try { await recordPlatformReactivationEvent(); } catch(_){ }
            }
          } else {
        ...
        try {
          ...
          const threshold = resolveAmplifyThreshold(platform);
            if (!amplifiedAt && threshold && normalizedScore !== null && normalizedScore >= threshold) {
              ...
              try { await recordPlatformAmplifyTrigger(); } catch(_){}
              // Enqueue cross-platform amplification tasks (exclude current platform)
              const targets = ['facebook','twitter','instagram','tiktok'].filter(p => p !== platform);
              for (const target of targets) {
                ...
                } catch(enqErr){ logger.warn('amplify_enqueue_failed', { error: enqErr.message, target, sourcePlatform: platform }); }
              }
              logger.info('platform_amplify_trigger', { platform, contentId, normalizedScore, threshold });
            }
        } catch (ampErr) {
        ...
        try {
          if (d.contentId && typeof normalizedScore === 'number') {
            ...
            const unified = Object.values(newScores).reduce((m, v) => typeof v === 'number' ? Math.max(m, v) : m, 0);
            await contentRef.set({ amplificationScores: newScores, amplificationUnified: unified, amplificationUpdatedAt: admin.firestore.FieldValue.serverTimestamp() }, { merge: true });
          }
        } catch(_) {}
        processed++;
      }
