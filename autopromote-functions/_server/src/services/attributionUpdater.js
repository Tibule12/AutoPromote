  ...
  try {
    ...
    await db.runTransaction(async tx => {
      ...
      if (typeof variantIndex === "number") {
        ...
        updates[key] =
          ((snap.exists && snap.data().variantClicks && snap.data().variantClicks[variantIndex]) ||
            0) + 1;
      }
      if (usedVariant) {
        ...
        updates[k2] =
          ((snap.exists &&
            snap.data().variantStringClicks &&
            snap.data().variantStringClicks[usedVariant]) ||
            0) + 1;
      }
    ...
    if (code) {
      try {
        ...
        if (!postSnap.empty) {
          ...
          await ref.update({
            clicks: (postSnap.docs[0].data().clicks || 0) + 1,
