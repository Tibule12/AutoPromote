...
async function postToYouTube({ message, url, youtubeAccessToken, videoFileUrl, title, description, uid }) {
  // Backwards compatibility: if no videoFileUrl, simulate (Phase 1 limitation inside functions env)
  if (!videoFileUrl || !uid) {
    return { success: true, platform: 'youtube', simulated: true, note: 'No videoFileUrl/uid provided; upload skipped.' };
  }
  try {
    ...
    try {
      ...
      return { success: true, platform: 'youtube', simulated: true, note: 'youtubeService not accessible from functions runtime.' };
    }
