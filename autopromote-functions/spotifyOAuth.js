...
exports.spotifyOAuthCallback = functions.region(region).https.onRequest(async (req, res) => {
  ...
  try {
    if (!(clientId && clientSecret && redirectUri)) {
      try { await admin.firestore().collection('oauth_states').doc(state || 'anon').set({ lastCallback: Date.now(), platform: 'spotify', placeholder:true }, { merge: true }); } catch(_){}
      return res.status(200).send('Spotify callback received; server missing client config for token exchange.');
    }
    const tokenUrl = 'https://accounts.spotify.com/api/token';
    const body = new URLSearchParams({ grant_type: 'authorization_code', code, redirect_uri: redirectUri });
    const auth = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');
    const tokenRes = await fetch(tokenUrl, { method: 'POST', headers: { Authorization: `Basic ${auth}`, 'Content-Type': 'application/x-www-form-urlencoded' }, body });
    ...
    if (uid && uid !== 'anon') {
      await admin.firestore().collection('users').doc(uid).collection('connections').doc('spotify').set(storeData, { merge: true });
    } else {
      await admin.firestore().collection('spotify_tokens').add({ tokenJson: encryptToken(JSON.stringify(tokenJson)), createdAt: Date.now() });
    }
    return res.status(200).send('Spotify OAuth callback received. You can close this window.');
  } catch (e) {
    console.error('Spotify callback error', e);
    return res.status(500).send('Spotify callback error: ' + (e && e.message ? e.message : 'unknown'));
  }
});
