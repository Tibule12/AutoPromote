...
exports.addReferrerToContent = functions.region(region).https.onCall(async (data, context) => {
  ...
  if (!contentId || !referrerId) {
    throw new functions.https.HttpsError('invalid-argument', 'contentId and referrerId are required');
  }
  try {
...
exports.getReferralStats = functions.region(region).https.onCall(async (data, context) => {
  ...
  try {
    ...
    const contentSnapshot = await admin.firestore().collection('content').where('referrerId', '==', referrerId).get();
    const contentIds = contentSnapshot.docs.map(doc => doc.id);
    // Get analytics events for these contentIds
    let totalTraffic = 0;
    let totalRevenue = 0;
    if (contentIds.length > 0) {
      const analyticsSnapshot = await admin.firestore().collection('analytics').where('contentId', 'in', contentIds.slice(0, 10)).get();
