...
exports.snapchatOAuthCallback = functions.region(region).https.onRequest(async (req, res) => {
  ...
  try {
    if (!(clientId && clientSecret && redirectUri)) {
      try { await admin.firestore().collection('oauth_states').doc(state || 'anon').set({ lastCallback: Date.now(), platform: 'snapchat', placeholder:true }, { merge: true }); } catch(_){}
      return res.status(200).send('Snapchat callback received; server missing client config for token exchange.');
    }
    ...
    if (uid && uid !== 'anon') { await admin.firestore().collection('users').doc(uid).collection('connections').doc('snapchat').set(storeData, { merge: true }); } else { await admin.firestore().collection('snapchat_tokens').add({ tokenJson: encryptToken(JSON.stringify(tokenJson)), createdAt: Date.now() }); }
    return res.status(200).send('Snapchat OAuth callback received. You can close this window.');
  } catch (e) { console.error('Snapchat callback error', e); return res.status(500).send('Snapchat callback error: ' + (e && e.message ? e.message : 'unknown')); }
});
