...
async function runProductionTests() {
  ...
  const platforms = [
    'youtube', 'twitter', 'facebook', 'tiktok', 'telegram',
    'linkedin', 'pinterest', 'reddit', 'discord', 'spotify', 'snapchat'
  ];
  ...
  for (const platform of platforms) {
    ...
    try {
      ...
      if (response.status === 200 || response.status === 401 || response.status === 403) {
        ...
        log(`❌ ${platform.charAt(0).toUpperCase() + platform.slice(1)} unexpected status: ${response.status}`, 'red');
      }
    } catch (err) {
