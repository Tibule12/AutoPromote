...
class OptimizationService {
  ...
  getTimeOfDayMultiplier(hour, platform) {
    const timePatterns = {
      youtube: [
        0.8, 0.7, 0.6, 0.5, 0.6, 0.8, 1.2, 1.4, 1.6, 1.8, 1.9, 2.0, 1.8, 1.6, 1.4, 1.2, 1.4, 1.6,
        1.8, 2.0, 1.8, 1.6, 1.4, 1.2,
      ],
      tiktok: [
        1.2, 1.1, 1.0, 0.9, 0.8, 0.9, 1.1, 1.3, 1.5, 1.6, 1.7, 1.8, 1.6, 1.4, 1.2, 1.1, 1.2, 1.4,
        1.6, 1.8, 2.0, 1.8, 1.6, 1.4,
      ],
      instagram: [
        0.9, 0.8, 0.7, 0.6, 0.7, 0.9, 1.2, 1.4, 1.6, 1.7, 1.8, 1.9, 1.7, 1.5, 1.3, 1.2, 1.3, 1.5,
        1.7, 1.9, 1.8, 1.6, 1.4, 1.1,
      ],
      facebook: [
        1.0, 0.9, 0.8, 0.7, 0.8, 1.0, 1.3, 1.5, 1.7, 1.8, 1.7, 1.6, 1.4, 1.2, 1.1, 1.0, 1.1, 1.3,
        1.5, 1.7, 1.6, 1.4, 1.2, 1.0,
      ],
    };
  ...
  calculateExpectedROI(content, platform, budget, riskProfile = "moderate") {
    ...
    const confidenceScore = this.calculateConfidenceScore(content, platform, historicalData);
  ...
  calculateConfidenceScore(content, platform, historicalData = {}) {
    ...
    if (historicalData.views && historicalData.views > 1000) {
      score += 0.1;
    }

    if (historicalData.conversion_rate && historicalData.conversion_rate > 0.01) {
      score += 0.1;
    }

    if (historicalData.engagement_rate && historicalData.engagement_rate > 0.08) {
      score += 0.1;
    }
  ...
  generateOptimizationRecommendations(content, analyticsData = {}) {
    ...
    if (content.max_budget) {
      ...
      platforms.forEach(platform => {
        platformBudgets[platform] = this.calculateOptimalBudget(
          content,
          { platform },
          analyticsData
        );
      ...
      const totalRecommended = Object.values(platformBudgets).reduce(
  ...
  calculateExpectedPerformance(variation, platform) {
    ...
    return {
      ...
      expected_roi:
        ((revenue - (variation.max_budget || 1000)) / (variation.max_budget || 1000)) * 100,
    };
  }
  ...
  calculateTestConfidence(resultsA, resultsB, sampleSize) {
    ...
    const stdDev =
      Math.sqrt(resultsA.expected_revenue + resultsB.expected_revenue) / Math.sqrt(sampleSize);
